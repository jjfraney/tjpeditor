/*
* generated by Xtext
*/
package org.jjflyboy.tjpeditor.ui.contentassist.antlr;

import java.util.Collection;
import java.util.Map;
import java.util.HashMap;

import org.antlr.runtime.RecognitionException;
import org.eclipse.xtext.AbstractElement;
import org.eclipse.xtext.ui.editor.contentassist.antlr.AbstractContentAssistParser;
import org.eclipse.xtext.ui.editor.contentassist.antlr.FollowElement;
import org.eclipse.xtext.ui.editor.contentassist.antlr.internal.AbstractInternalContentAssistParser;

import com.google.inject.Inject;

import org.jjflyboy.tjpeditor.services.ProjectGrammarAccess;

public class ProjectParser extends AbstractContentAssistParser {
	
	@Inject
	private ProjectGrammarAccess grammarAccess;
	
	private Map<AbstractElement, String> nameMappings;
	
	@Override
	protected org.jjflyboy.tjpeditor.ui.contentassist.antlr.internal.InternalProjectParser createParser() {
		org.jjflyboy.tjpeditor.ui.contentassist.antlr.internal.InternalProjectParser result = new org.jjflyboy.tjpeditor.ui.contentassist.antlr.internal.InternalProjectParser(null);
		result.setGrammarAccess(grammarAccess);
		return result;
	}
	
	@Override
	protected String getRuleName(AbstractElement element) {
		if (nameMappings == null) {
			nameMappings = new HashMap<AbstractElement, String>() {
				private static final long serialVersionUID = 1L;
				{
					put(grammarAccess.getPropertyAccess().getAlternatives(), "rule__Property__Alternatives");
					put(grammarAccess.getAccountAttributeAccess().getAlternatives(), "rule__AccountAttribute__Alternatives");
					put(grammarAccess.getProjectAttributeAccess().getAlternatives(), "rule__ProjectAttribute__Alternatives");
					put(grammarAccess.getTaskAttributeAccess().getAlternatives(), "rule__TaskAttribute__Alternatives");
					put(grammarAccess.getReportAttributeAccess().getAlternatives(), "rule__ReportAttribute__Alternatives");
					put(grammarAccess.getIcalReportAttributeAccess().getAlternatives(), "rule__IcalReportAttribute__Alternatives");
					put(grammarAccess.getExportAttributeAccess().getAlternatives(), "rule__ExportAttribute__Alternatives");
					put(grammarAccess.getResourceAttributeAccess().getAlternatives(), "rule__ResourceAttribute__Alternatives");
					put(grammarAccess.getAllocateResourceAttributeAccess().getAlternatives(), "rule__AllocateResourceAttribute__Alternatives");
					put(grammarAccess.getNewTaskAttributeAccess().getAlternatives(), "rule__NewTaskAttribute__Alternatives");
					put(grammarAccess.getNikuReportAttributeAccess().getAlternatives(), "rule__NikuReportAttribute__Alternatives");
					put(grammarAccess.getDefinitionsAccess().getAlternatives_1(), "rule__Definitions__Alternatives_1");
					put(grammarAccess.getExtendAccess().getAlternatives_0(), "rule__Extend__Alternatives_0");
					put(grammarAccess.getFunctionAccess().getAlternatives_1(), "rule__Function__Alternatives_1");
					put(grammarAccess.getFunctionAccess().getAlternatives_1_0_0(), "rule__Function__Alternatives_1_0_0");
					put(grammarAccess.getFunctionAccess().getAlternatives_1_1_0(), "rule__Function__Alternatives_1_1_0");
					put(grammarAccess.getFunctionAccess().getAlternatives_1_2_0(), "rule__Function__Alternatives_1_2_0");
					put(grammarAccess.getFunctionAccess().getAlternatives_1_3_0(), "rule__Function__Alternatives_1_3_0");
					put(grammarAccess.getFunctionAccess().getAlternatives_1_4_0(), "rule__Function__Alternatives_1_4_0");
					put(grammarAccess.getFunctionAccess().getAlternatives_1_5_0(), "rule__Function__Alternatives_1_5_0");
					put(grammarAccess.getFunctionAccess().getAlternatives_1_6_0(), "rule__Function__Alternatives_1_6_0");
					put(grammarAccess.getFunctionAccess().getAlternatives_1_7_0(), "rule__Function__Alternatives_1_7_0");
					put(grammarAccess.getFunctionAccess().getAlternatives_1_8_0(), "rule__Function__Alternatives_1_8_0");
					put(grammarAccess.getFunctionAccess().getAlternatives_1_9_0(), "rule__Function__Alternatives_1_9_0");
					put(grammarAccess.getFunctionAccess().getAlternatives_1_10_0(), "rule__Function__Alternatives_1_10_0");
					put(grammarAccess.getFunctionAccess().getAlternatives_1_11_0(), "rule__Function__Alternatives_1_11_0");
					put(grammarAccess.getFunctionAccess().getAlternatives_1_12_0(), "rule__Function__Alternatives_1_12_0");
					put(grammarAccess.getIncludePropertiesAttributeAccess().getAlternatives(), "rule__IncludePropertiesAttribute__Alternatives");
					put(grammarAccess.getInterval1Access().getAlternatives_1(), "rule__Interval1__Alternatives_1");
					put(grammarAccess.getInterval2Access().getAlternatives_1(), "rule__Interval2__Alternatives_1");
					put(grammarAccess.getInterval3Access().getAlternatives_1(), "rule__Interval3__Alternatives_1");
					put(grammarAccess.getInterval4Access().getAlternatives_1(), "rule__Interval4__Alternatives_1");
					put(grammarAccess.getJournalAttributesAccess().getAlternatives_1(), "rule__JournalAttributes__Alternatives_1");
					put(grammarAccess.getLimitsAttributeAccess().getAlternatives(), "rule__LimitsAttribute__Alternatives");
					put(grammarAccess.getLEOpEqualityAccess().getAlternatives(), "rule__LEOpEquality__Alternatives");
					put(grammarAccess.getLogicalPrimaryExpressionAccess().getAlternatives(), "rule__LogicalPrimaryExpression__Alternatives");
					put(grammarAccess.getLogicalBooleanLiteralAccess().getAlternatives_1(), "rule__LogicalBooleanLiteral__Alternatives_1");
					put(grammarAccess.getResourceAttributesAccess().getAlternatives_1(), "rule__ResourceAttributes__Alternatives_1");
					put(grammarAccess.getScaleAccess().getScaleAlternatives_1_0(), "rule__Scale__ScaleAlternatives_1_0");
					put(grammarAccess.getSortAccess().getAlternatives_1(), "rule__Sort__Alternatives_1");
					put(grammarAccess.getStatusStatusSheetAttributeAccess().getAlternatives(), "rule__StatusStatusSheetAttribute__Alternatives");
					put(grammarAccess.getStatusTimesheetAttributeAccess().getAlternatives(), "rule__StatusTimesheetAttribute__Alternatives");
					put(grammarAccess.getStatusSheetReportAttributeAccess().getAlternatives(), "rule__StatusSheetReportAttribute__Alternatives");
					put(grammarAccess.getTaskStatusSheetAttributeAccess().getAlternatives(), "rule__TaskStatusSheetAttribute__Alternatives");
					put(grammarAccess.getTaskTimesheetAttributeAccess().getAlternatives(), "rule__TaskTimesheetAttribute__Alternatives");
					put(grammarAccess.getTaskAttributesAccess().getAlternatives_1(), "rule__TaskAttributes__Alternatives_1");
					put(grammarAccess.getTimesheetAttributeAccess().getAlternatives(), "rule__TimesheetAttribute__Alternatives");
					put(grammarAccess.getTimesheetReportAttributeAccess().getAlternatives(), "rule__TimesheetReportAttribute__Alternatives");
					put(grammarAccess.getWeekStartsAccess().getAlternatives(), "rule__WeekStarts__Alternatives");
					put(grammarAccess.getWorkingHoursAccess().getAlternatives_2(), "rule__WorkingHours__Alternatives_2");
					put(grammarAccess.getColumnAttributeAccess().getAlternatives(), "rule__ColumnAttribute__Alternatives");
					put(grammarAccess.getLimitAttributeAccess().getAlternatives(), "rule__LimitAttribute__Alternatives");
					put(grammarAccess.getXFloatAccess().getAlternatives(), "rule__XFloat__Alternatives");
					put(grammarAccess.getXAssignmentAccess().getAlternatives(), "rule__XAssignment__Alternatives");
					put(grammarAccess.getOpEqualityAccess().getAlternatives(), "rule__OpEquality__Alternatives");
					put(grammarAccess.getXRelationalExpressionAccess().getAlternatives_1(), "rule__XRelationalExpression__Alternatives_1");
					put(grammarAccess.getOpCompareAccess().getAlternatives(), "rule__OpCompare__Alternatives");
					put(grammarAccess.getOpOtherAccess().getAlternatives(), "rule__OpOther__Alternatives");
					put(grammarAccess.getOpAddAccess().getAlternatives(), "rule__OpAdd__Alternatives");
					put(grammarAccess.getOpMultiAccess().getAlternatives(), "rule__OpMulti__Alternatives");
					put(grammarAccess.getXUnaryOperationAccess().getAlternatives(), "rule__XUnaryOperation__Alternatives");
					put(grammarAccess.getOpUnaryAccess().getAlternatives(), "rule__OpUnary__Alternatives");
					put(grammarAccess.getXMemberFeatureCallAccess().getAlternatives_1(), "rule__XMemberFeatureCall__Alternatives_1");
					put(grammarAccess.getXMemberFeatureCallAccess().getAlternatives_1_1_0_0_1(), "rule__XMemberFeatureCall__Alternatives_1_1_0_0_1");
					put(grammarAccess.getXMemberFeatureCallAccess().getAlternatives_1_1_3_1(), "rule__XMemberFeatureCall__Alternatives_1_1_3_1");
					put(grammarAccess.getXPrimaryExpressionAccess().getAlternatives(), "rule__XPrimaryExpression__Alternatives");
					put(grammarAccess.getXLiteralAccess().getAlternatives(), "rule__XLiteral__Alternatives");
					put(grammarAccess.getXExpressionInsideBlockAccess().getAlternatives(), "rule__XExpressionInsideBlock__Alternatives");
					put(grammarAccess.getXVariableDeclarationAccess().getAlternatives_1(), "rule__XVariableDeclaration__Alternatives_1");
					put(grammarAccess.getXVariableDeclarationAccess().getAlternatives_2(), "rule__XVariableDeclaration__Alternatives_2");
					put(grammarAccess.getXFeatureCallAccess().getAlternatives_4_1(), "rule__XFeatureCall__Alternatives_4_1");
					put(grammarAccess.getIdOrSuperAccess().getAlternatives(), "rule__IdOrSuper__Alternatives");
					put(grammarAccess.getXConstructorCallAccess().getAlternatives_5(), "rule__XConstructorCall__Alternatives_5");
					put(grammarAccess.getXBooleanLiteralAccess().getAlternatives_1(), "rule__XBooleanLiteral__Alternatives_1");
					put(grammarAccess.getXTryCatchFinallyExpressionAccess().getAlternatives_3(), "rule__XTryCatchFinallyExpression__Alternatives_3");
					put(grammarAccess.getJvmTypeReferenceAccess().getAlternatives(), "rule__JvmTypeReference__Alternatives");
					put(grammarAccess.getJvmArgumentTypeReferenceAccess().getAlternatives(), "rule__JvmArgumentTypeReference__Alternatives");
					put(grammarAccess.getJvmWildcardTypeReferenceAccess().getAlternatives_2(), "rule__JvmWildcardTypeReference__Alternatives_2");
					put(grammarAccess.getJvmTypeParameterAccess().getAlternatives_1(), "rule__JvmTypeParameter__Alternatives_1");
					put(grammarAccess.getPurgeReportAttributeAccess().getAlternatives(), "rule__PurgeReportAttribute__Alternatives");
					put(grammarAccess.getPurgeResourceAttributeAccess().getAlternatives(), "rule__PurgeResourceAttribute__Alternatives");
					put(grammarAccess.getPurgeTaskAttributeAccess().getAlternatives(), "rule__PurgeTaskAttribute__Alternatives");
					put(grammarAccess.getChargeAppliesAccess().getAlternatives(), "rule__ChargeApplies__Alternatives");
					put(grammarAccess.getJustificationAccess().getAlternatives(), "rule__Justification__Alternatives");
					put(grammarAccess.getJournalModeValueAccess().getAlternatives(), "rule__JournalModeValue__Alternatives");
					put(grammarAccess.getJournalEntrySortCriterionAccess().getAlternatives(), "rule__JournalEntrySortCriterion__Alternatives");
					put(grammarAccess.getListTypeValuesAccess().getAlternatives(), "rule__ListTypeValues__Alternatives");
					put(grammarAccess.getCriterionDirectionAccess().getAlternatives(), "rule__CriterionDirection__Alternatives");
					put(grammarAccess.getYesNoAccess().getAlternatives(), "rule__YesNo__Alternatives");
					put(grammarAccess.getReportFormatAccess().getAlternatives(), "rule__ReportFormat__Alternatives");
					put(grammarAccess.getLoadDisplayUnitAccess().getAlternatives(), "rule__LoadDisplayUnit__Alternatives");
					put(grammarAccess.getScaleResolutionAccess().getAlternatives(), "rule__ScaleResolution__Alternatives");
					put(grammarAccess.getSelectArgumentAccess().getAlternatives(), "rule__SelectArgument__Alternatives");
					put(grammarAccess.getColumnIdAccess().getAlternatives(), "rule__ColumnId__Alternatives");
					put(grammarAccess.getAlertLevelAccess().getAlternatives(), "rule__AlertLevel__Alternatives");
					put(grammarAccess.getDependsPolicyAccess().getAlternatives(), "rule__DependsPolicy__Alternatives");
					put(grammarAccess.getSchedulingPolicyAccess().getAlternatives(), "rule__SchedulingPolicy__Alternatives");
					put(grammarAccess.getTimeUnitAccess().getAlternatives(), "rule__TimeUnit__Alternatives");
					put(grammarAccess.getWeekdayAccess().getAlternatives(), "rule__Weekday__Alternatives");
					put(grammarAccess.getWorkQuantityUnitAccess().getAlternatives(), "rule__WorkQuantityUnit__Alternatives");
					put(grammarAccess.getGlobalAccess().getGroup(), "rule__Global__Group__0");
					put(grammarAccess.getAbsoluteIdAccess().getGroup(), "rule__AbsoluteId__Group__0");
					put(grammarAccess.getAbsoluteIdAccess().getGroup_1(), "rule__AbsoluteId__Group_1__0");
					put(grammarAccess.getAccountAccess().getGroup(), "rule__Account__Group__0");
					put(grammarAccess.getAccountAccess().getGroup_3(), "rule__Account__Group_3__0");
					put(grammarAccess.getAccountPrefixAccess().getGroup(), "rule__AccountPrefix__Group__0");
					put(grammarAccess.getAccountReportAccess().getGroup(), "rule__AccountReport__Group__0");
					put(grammarAccess.getAccountRootAccess().getGroup(), "rule__AccountRoot__Group__0");
					put(grammarAccess.getProjectAccess().getGroup(), "rule__Project__Group__0");
					put(grammarAccess.getProjectAccess().getGroup_5(), "rule__Project__Group_5__0");
					put(grammarAccess.getTaskAccess().getGroup(), "rule__Task__Group__0");
					put(grammarAccess.getTaskAccess().getGroup_3(), "rule__Task__Group_3__0");
					put(grammarAccess.getReportAccess().getGroup(), "rule__Report__Group__0");
					put(grammarAccess.getReportAccess().getGroup_2(), "rule__Report__Group_2__0");
					put(grammarAccess.getIcalReportAccess().getGroup(), "rule__IcalReport__Group__0");
					put(grammarAccess.getIcalReportAccess().getGroup_2(), "rule__IcalReport__Group_2__0");
					put(grammarAccess.getExportAccess().getGroup(), "rule__Export__Group__0");
					put(grammarAccess.getExportAccess().getGroup_3(), "rule__Export__Group_3__0");
					put(grammarAccess.getResourceAccess().getGroup(), "rule__Resource__Group__0");
					put(grammarAccess.getResourceAccess().getGroup_3(), "rule__Resource__Group_3__0");
					put(grammarAccess.getAllocateAccess().getGroup(), "rule__Allocate__Group__0");
					put(grammarAccess.getAllocateAccess().getGroup_2(), "rule__Allocate__Group_2__0");
					put(grammarAccess.getAllocateResourceAccess().getGroup(), "rule__AllocateResource__Group__0");
					put(grammarAccess.getAllocateResourceAccess().getGroup_1(), "rule__AllocateResource__Group_1__0");
					put(grammarAccess.getNavigatorAccess().getGroup(), "rule__Navigator__Group__0");
					put(grammarAccess.getNavigatorAccess().getGroup_2(), "rule__Navigator__Group_2__0");
					put(grammarAccess.getNewTaskAccess().getGroup(), "rule__NewTask__Group__0");
					put(grammarAccess.getNewTaskAccess().getGroup_3(), "rule__NewTask__Group_3__0");
					put(grammarAccess.getNikuReportAccess().getGroup(), "rule__NikuReport__Group__0");
					put(grammarAccess.getNikuReportAccess().getGroup_2(), "rule__NikuReport__Group_2__0");
					put(grammarAccess.getAlertAccess().getGroup(), "rule__Alert__Group__0");
					put(grammarAccess.getAlternativeAccess().getGroup(), "rule__Alternative__Group__0");
					put(grammarAccess.getAlternativeAccess().getGroup_1(), "rule__Alternative__Group_1__0");
					put(grammarAccess.getAuthorAccess().getGroup(), "rule__Author__Group__0");
					put(grammarAccess.getBalanceAccess().getGroup(), "rule__Balance__Group__0");
					put(grammarAccess.getBookingAccess().getGroup(), "rule__Booking__Group__0");
					put(grammarAccess.getBookingAccess().getGroup_1(), "rule__Booking__Group_1__0");
					put(grammarAccess.getBookingAccess().getGroup_1_1_0(), "rule__Booking__Group_1_1_0__0");
					put(grammarAccess.getBookingAccess().getGroup_1_1_1(), "rule__Booking__Group_1_1_1__0");
					put(grammarAccess.getBookingTaskAccess().getGroup(), "rule__BookingTask__Group__0");
					put(grammarAccess.getBookingResourceAccess().getGroup(), "rule__BookingResource__Group__0");
					put(grammarAccess.getCaptionAccess().getGroup(), "rule__Caption__Group__0");
					put(grammarAccess.getCellColorAccess().getGroup(), "rule__CellColor__Group__0");
					put(grammarAccess.getCellTextAccess().getGroup(), "rule__CellText__Group__0");
					put(grammarAccess.getCenterAccess().getGroup(), "rule__Center__Group__0");
					put(grammarAccess.getChargeAccess().getGroup(), "rule__Charge__Group__0");
					put(grammarAccess.getChargeSetAccess().getGroup(), "rule__ChargeSet__Group__0");
					put(grammarAccess.getChargeSetAccess().getGroup_2(), "rule__ChargeSet__Group_2__0");
					put(grammarAccess.getColumnsAccess().getGroup(), "rule__Columns__Group__0");
					put(grammarAccess.getColumnsAccess().getGroup_2(), "rule__Columns__Group_2__0");
					put(grammarAccess.getCompleteAccess().getGroup(), "rule__Complete__Group__0");
					put(grammarAccess.getCopyrightAccess().getGroup(), "rule__Copyright__Group__0");
					put(grammarAccess.getCreditAccess().getGroup(), "rule__Credit__Group__0");
					put(grammarAccess.getCurrencyAccess().getGroup(), "rule__Currency__Group__0");
					put(grammarAccess.getCurrencyFormatAccess().getGroup(), "rule__CurrencyFormat__Group__0");
					put(grammarAccess.getDailyMaxAccess().getGroup(), "rule__DailyMax__Group__0");
					put(grammarAccess.getDailyMinAccess().getGroup(), "rule__DailyMin__Group__0");
					put(grammarAccess.getDailyWorkingHoursAccess().getGroup(), "rule__DailyWorkingHours__Group__0");
					put(grammarAccess.getDefinitionsAccess().getGroup(), "rule__Definitions__Group__0");
					put(grammarAccess.getDefinitionsAccess().getGroup_1_2(), "rule__Definitions__Group_1_2__0");
					put(grammarAccess.getDependsAccess().getGroup(), "rule__Depends__Group__0");
					put(grammarAccess.getDetailsAccess().getGroup(), "rule__Details__Group__0");
					put(grammarAccess.getDurationAccess().getGroup(), "rule__Duration__Group__0");
					put(grammarAccess.getEfficiencyAccess().getGroup(), "rule__Efficiency__Group__0");
					put(grammarAccess.getEffortAccess().getGroup(), "rule__Effort__Group__0");
					put(grammarAccess.getEmailAccess().getGroup(), "rule__Email__Group__0");
					put(grammarAccess.getEndAccess().getGroup(), "rule__End__Group__0");
					put(grammarAccess.getEndCreditAccess().getGroup(), "rule__EndCredit__Group__0");
					put(grammarAccess.getEpilogAccess().getGroup(), "rule__Epilog__Group__0");
					put(grammarAccess.getExtendAccess().getGroup(), "rule__Extend__Group__0");
					put(grammarAccess.getExtendAccess().getGroup_3(), "rule__Extend__Group_3__0");
					put(grammarAccess.getExtendResourceAccess().getGroup(), "rule__ExtendResource__Group__0");
					put(grammarAccess.getExtendResourceAccess().getGroup_3(), "rule__ExtendResource__Group_3__0");
					put(grammarAccess.getExtendedResourceAttributeAccess().getGroup(), "rule__ExtendedResourceAttribute__Group__0");
					put(grammarAccess.getExtendTaskAccess().getGroup(), "rule__ExtendTask__Group__0");
					put(grammarAccess.getExtendTaskAccess().getGroup_3(), "rule__ExtendTask__Group_3__0");
					put(grammarAccess.getExtendedTaskAttributeAccess().getGroup(), "rule__ExtendedTaskAttribute__Group__0");
					put(grammarAccess.getFailAccess().getGroup(), "rule__Fail__Group__0");
					put(grammarAccess.getFlagsAccess().getGroup(), "rule__Flags__Group__0");
					put(grammarAccess.getFlagsAccess().getGroup_2(), "rule__Flags__Group_2__0");
					put(grammarAccess.getFontColorAccess().getGroup(), "rule__FontColor__Group__0");
					put(grammarAccess.getFooterAccess().getGroup(), "rule__Footer__Group__0");
					put(grammarAccess.getFormatsAccess().getGroup(), "rule__Formats__Group__0");
					put(grammarAccess.getFormatsAccess().getGroup_2(), "rule__Formats__Group_2__0");
					put(grammarAccess.getFunctionAccess().getGroup(), "rule__Function__Group__0");
					put(grammarAccess.getFunctionAccess().getGroup_1_0(), "rule__Function__Group_1_0__0");
					put(grammarAccess.getFunctionAccess().getGroup_1_1(), "rule__Function__Group_1_1__0");
					put(grammarAccess.getFunctionAccess().getGroup_1_2(), "rule__Function__Group_1_2__0");
					put(grammarAccess.getFunctionAccess().getGroup_1_3(), "rule__Function__Group_1_3__0");
					put(grammarAccess.getFunctionAccess().getGroup_1_4(), "rule__Function__Group_1_4__0");
					put(grammarAccess.getFunctionAccess().getGroup_1_5(), "rule__Function__Group_1_5__0");
					put(grammarAccess.getFunctionAccess().getGroup_1_6(), "rule__Function__Group_1_6__0");
					put(grammarAccess.getFunctionAccess().getGroup_1_7(), "rule__Function__Group_1_7__0");
					put(grammarAccess.getFunctionAccess().getGroup_1_8(), "rule__Function__Group_1_8__0");
					put(grammarAccess.getFunctionAccess().getGroup_1_9(), "rule__Function__Group_1_9__0");
					put(grammarAccess.getFunctionAccess().getGroup_1_10(), "rule__Function__Group_1_10__0");
					put(grammarAccess.getFunctionAccess().getGroup_1_11(), "rule__Function__Group_1_11__0");
					put(grammarAccess.getFunctionAccess().getGroup_1_12(), "rule__Function__Group_1_12__0");
					put(grammarAccess.getGapDurationAccess().getGroup(), "rule__GapDuration__Group__0");
					put(grammarAccess.getGapLengthAccess().getGroup(), "rule__GapLength__Group__0");
					put(grammarAccess.getHAlignAccess().getGroup(), "rule__HAlign__Group__0");
					put(grammarAccess.getHeaderAccess().getGroup(), "rule__Header__Group__0");
					put(grammarAccess.getHeadlineAccess().getGroup(), "rule__Headline__Group__0");
					put(grammarAccess.getHideAccountAccess().getGroup(), "rule__HideAccount__Group__0");
					put(grammarAccess.getHideJournalEntryAccess().getGroup(), "rule__HideJournalEntry__Group__0");
					put(grammarAccess.getHideReportAccess().getGroup(), "rule__HideReport__Group__0");
					put(grammarAccess.getHideResourceAccess().getGroup(), "rule__HideResource__Group__0");
					put(grammarAccess.getHideTaskAccess().getGroup(), "rule__HideTask__Group__0");
					put(grammarAccess.getIncludeAccess().getGroup(), "rule__Include__Group__0");
					put(grammarAccess.getIncludePropertiesAccess().getGroup(), "rule__IncludeProperties__Group__0");
					put(grammarAccess.getIncludePropertiesAccess().getGroup_2(), "rule__IncludeProperties__Group_2__0");
					put(grammarAccess.getInterval1Access().getGroup(), "rule__Interval1__Group__0");
					put(grammarAccess.getInterval1Access().getGroup_1_0(), "rule__Interval1__Group_1_0__0");
					put(grammarAccess.getInterval1Access().getGroup_1_1(), "rule__Interval1__Group_1_1__0");
					put(grammarAccess.getInterval2Access().getGroup(), "rule__Interval2__Group__0");
					put(grammarAccess.getInterval2Access().getGroup_1_0(), "rule__Interval2__Group_1_0__0");
					put(grammarAccess.getInterval2Access().getGroup_1_1(), "rule__Interval2__Group_1_1__0");
					put(grammarAccess.getInterval3Access().getGroup(), "rule__Interval3__Group__0");
					put(grammarAccess.getInterval3Access().getGroup_1_0(), "rule__Interval3__Group_1_0__0");
					put(grammarAccess.getInterval3Access().getGroup_1_1(), "rule__Interval3__Group_1_1__0");
					put(grammarAccess.getInterval4Access().getGroup(), "rule__Interval4__Group__0");
					put(grammarAccess.getInterval4Access().getGroup_1_0(), "rule__Interval4__Group_1_0__0");
					put(grammarAccess.getInterval4Access().getGroup_1_1(), "rule__Interval4__Group_1_1__0");
					put(grammarAccess.getJournalAttributesAccess().getGroup(), "rule__JournalAttributes__Group__0");
					put(grammarAccess.getJournalAttributesAccess().getGroup_1_2(), "rule__JournalAttributes__Group_1_2__0");
					put(grammarAccess.getJournalEntryAccess().getGroup(), "rule__JournalEntry__Group__0");
					put(grammarAccess.getJournalEntryAccess().getGroup_3(), "rule__JournalEntry__Group_3__0");
					put(grammarAccess.getJournalModeAccess().getGroup(), "rule__JournalMode__Group__0");
					put(grammarAccess.getLeftAccess().getGroup(), "rule__Left__Group__0");
					put(grammarAccess.getLengthAccess().getGroup(), "rule__Length__Group__0");
					put(grammarAccess.getLimitsAccess().getGroup(), "rule__Limits__Group__0");
					put(grammarAccess.getLimitsAccess().getGroup_2(), "rule__Limits__Group_2__0");
					put(grammarAccess.getListItemAccess().getGroup(), "rule__ListItem__Group__0");
					put(grammarAccess.getListTypeAccess().getGroup(), "rule__ListType__Group__0");
					put(grammarAccess.getLoadUnitAccess().getGroup(), "rule__LoadUnit__Group__0");
					put(grammarAccess.getLogicalOrExpressionAccess().getGroup(), "rule__LogicalOrExpression__Group__0");
					put(grammarAccess.getLogicalOrExpressionAccess().getGroup_1(), "rule__LogicalOrExpression__Group_1__0");
					put(grammarAccess.getLogicalOrExpressionAccess().getGroup_1_0(), "rule__LogicalOrExpression__Group_1_0__0");
					put(grammarAccess.getLogicalOrExpressionAccess().getGroup_1_0_0(), "rule__LogicalOrExpression__Group_1_0_0__0");
					put(grammarAccess.getLogicalAndExpressionAccess().getGroup(), "rule__LogicalAndExpression__Group__0");
					put(grammarAccess.getLogicalAndExpressionAccess().getGroup_1(), "rule__LogicalAndExpression__Group_1__0");
					put(grammarAccess.getLogicalAndExpressionAccess().getGroup_1_0(), "rule__LogicalAndExpression__Group_1_0__0");
					put(grammarAccess.getLogicalAndExpressionAccess().getGroup_1_0_0(), "rule__LogicalAndExpression__Group_1_0_0__0");
					put(grammarAccess.getLogicalEqualityExpressionAccess().getGroup(), "rule__LogicalEqualityExpression__Group__0");
					put(grammarAccess.getLogicalEqualityExpressionAccess().getGroup_1(), "rule__LogicalEqualityExpression__Group_1__0");
					put(grammarAccess.getLogicalEqualityExpressionAccess().getGroup_1_0(), "rule__LogicalEqualityExpression__Group_1_0__0");
					put(grammarAccess.getLogicalEqualityExpressionAccess().getGroup_1_0_0(), "rule__LogicalEqualityExpression__Group_1_0_0__0");
					put(grammarAccess.getLogicalRelationalExpressionAccess().getGroup(), "rule__LogicalRelationalExpression__Group__0");
					put(grammarAccess.getLogicalRelationalExpressionAccess().getGroup_1(), "rule__LogicalRelationalExpression__Group_1__0");
					put(grammarAccess.getLogicalRelationalExpressionAccess().getGroup_1_0(), "rule__LogicalRelationalExpression__Group_1_0__0");
					put(grammarAccess.getLogicalRelationalExpressionAccess().getGroup_1_0_0(), "rule__LogicalRelationalExpression__Group_1_0_0__0");
					put(grammarAccess.getLogicalFunctionExpressionAccess().getGroup(), "rule__LogicalFunctionExpression__Group__0");
					put(grammarAccess.getLogicalAbsoluteIdExpressionAccess().getGroup(), "rule__LogicalAbsoluteIdExpression__Group__0");
					put(grammarAccess.getLogicalBooleanLiteralAccess().getGroup(), "rule__LogicalBooleanLiteral__Group__0");
					put(grammarAccess.getLogicalNumeralLiteralAccess().getGroup(), "rule__LogicalNumeralLiteral__Group__0");
					put(grammarAccess.getLogicalStringLiteralAccess().getGroup(), "rule__LogicalStringLiteral__Group__0");
					put(grammarAccess.getLogicalISODateLiteralAccess().getGroup(), "rule__LogicalISODateLiteral__Group__0");
					put(grammarAccess.getLogicalPrenthesizedExpressionAccess().getGroup(), "rule__LogicalPrenthesizedExpression__Group__0");
					put(grammarAccess.getMacroAccess().getGroup(), "rule__Macro__Group__0");
					put(grammarAccess.getManagersAccess().getGroup(), "rule__Managers__Group__0");
					put(grammarAccess.getManagersAccess().getGroup_2(), "rule__Managers__Group_2__0");
					put(grammarAccess.getMaxEndAccess().getGroup(), "rule__MaxEnd__Group__0");
					put(grammarAccess.getMaximumAccess().getGroup(), "rule__Maximum__Group__0");
					put(grammarAccess.getMaxStartAccess().getGroup(), "rule__MaxStart__Group__0");
					put(grammarAccess.getMinimumAccess().getGroup(), "rule__Minimum__Group__0");
					put(grammarAccess.getMinEndAccess().getGroup(), "rule__MinEnd__Group__0");
					put(grammarAccess.getMinStartAccess().getGroup(), "rule__MinStart__Group__0");
					put(grammarAccess.getMonthlyMaxAccess().getGroup(), "rule__MonthlyMax__Group__0");
					put(grammarAccess.getMonthlyMinAccess().getGroup(), "rule__MonthlyMin__Group__0");
					put(grammarAccess.getNoteAccess().getGroup(), "rule__Note__Group__0");
					put(grammarAccess.getNowAccess().getGroup(), "rule__Now__Group__0");
					put(grammarAccess.getNumberFormatAccess().getGroup(), "rule__NumberFormat__Group__0");
					put(grammarAccess.getPeriodAccess().getGroup(), "rule__Period__Group__0");
					put(grammarAccess.getPrecedesAccess().getGroup(), "rule__Precedes__Group__0");
					put(grammarAccess.getPriorityAccess().getGroup(), "rule__Priority__Group__0");
					put(grammarAccess.getProjectIdAccess().getGroup(), "rule__ProjectId__Group__0");
					put(grammarAccess.getProjectIdsAccess().getGroup(), "rule__ProjectIds__Group__0");
					put(grammarAccess.getProjectIdsAccess().getGroup_2(), "rule__ProjectIds__Group_2__0");
					put(grammarAccess.getPrologAccess().getGroup(), "rule__Prolog__Group__0");
					put(grammarAccess.getPurgeReportAccess().getGroup(), "rule__PurgeReport__Group__0");
					put(grammarAccess.getPurgeResourceAccess().getGroup(), "rule__PurgeResource__Group__0");
					put(grammarAccess.getPurgeTaskAccess().getGroup(), "rule__PurgeTask__Group__0");
					put(grammarAccess.getRateAccess().getGroup(), "rule__Rate__Group__0");
					put(grammarAccess.getRemainingAccess().getGroup(), "rule__Remaining__Group__0");
					put(grammarAccess.getReportPrefixAccess().getGroup(), "rule__ReportPrefix__Group__0");
					put(grammarAccess.getResourceAttributesAccess().getGroup(), "rule__ResourceAttributes__Group__0");
					put(grammarAccess.getResourceAttributesAccess().getGroup_1_2(), "rule__ResourceAttributes__Group_1_2__0");
					put(grammarAccess.getResourcePrefixAccess().getGroup(), "rule__ResourcePrefix__Group__0");
					put(grammarAccess.getResourceReportAccess().getGroup(), "rule__ResourceReport__Group__0");
					put(grammarAccess.getResourceRootAccess().getGroup(), "rule__ResourceRoot__Group__0");
					put(grammarAccess.getResponsibleAccess().getGroup(), "rule__Responsible__Group__0");
					put(grammarAccess.getResponsibleAccess().getGroup_2(), "rule__Responsible__Group_2__0");
					put(grammarAccess.getRightAccess().getGroup(), "rule__Right__Group__0");
					put(grammarAccess.getRollupAccountAccess().getGroup(), "rule__RollupAccount__Group__0");
					put(grammarAccess.getRollupResourceAccess().getGroup(), "rule__RollupResource__Group__0");
					put(grammarAccess.getRollupTaskAccess().getGroup(), "rule__RollupTask__Group__0");
					put(grammarAccess.getScaleAccess().getGroup(), "rule__Scale__Group__0");
					put(grammarAccess.getScenarioAccess().getGroup(), "rule__Scenario__Group__0");
					put(grammarAccess.getScenarioAccess().getGroup_3(), "rule__Scenario__Group_3__0");
					put(grammarAccess.getScenarioAccess().getGroup_3_1_0(), "rule__Scenario__Group_3_1_0__0");
					put(grammarAccess.getScenarioIcalAccess().getGroup(), "rule__ScenarioIcal__Group__0");
					put(grammarAccess.getScenariosAccess().getGroup(), "rule__Scenarios__Group__0");
					put(grammarAccess.getScenariosAccess().getGroup_2(), "rule__Scenarios__Group_2__0");
					put(grammarAccess.getSchedulingAccess().getGroup(), "rule__Scheduling__Group__0");
					put(grammarAccess.getSelectAccess().getGroup(), "rule__Select__Group__0");
					put(grammarAccess.getSelfContainedAccess().getGroup(), "rule__SelfContained__Group__0");
					put(grammarAccess.getShiftAccess().getGroup(), "rule__Shift__Group__0");
					put(grammarAccess.getShiftAccess().getGroup_3(), "rule__Shift__Group_3__0");
					put(grammarAccess.getShiftAccess().getGroup_3_1_1(), "rule__Shift__Group_3_1_1__0");
					put(grammarAccess.getShiftAccess().getGroup_3_1_2(), "rule__Shift__Group_3_1_2__0");
					put(grammarAccess.getShiftAccess().getGroup_3_1_3(), "rule__Shift__Group_3_1_3__0");
					put(grammarAccess.getShiftAccess().getGroup_3_1_4(), "rule__Shift__Group_3_1_4__0");
					put(grammarAccess.getShiftTimesheetAccess().getGroup(), "rule__ShiftTimesheet__Group__0");
					put(grammarAccess.getShiftsAccess().getGroup(), "rule__Shifts__Group__0");
					put(grammarAccess.getShiftsAccess().getGroup_2(), "rule__Shifts__Group_2__0");
					put(grammarAccess.getShiftsLimitAccess().getGroup(), "rule__ShiftsLimit__Group__0");
					put(grammarAccess.getShiftsAllocateAccess().getGroup(), "rule__ShiftsAllocate__Group__0");
					put(grammarAccess.getShiftsAllocateAccess().getGroup_2(), "rule__ShiftsAllocate__Group_2__0");
					put(grammarAccess.getShiftsAllocateAccess().getGroup_2_1(), "rule__ShiftsAllocate__Group_2_1__0");
					put(grammarAccess.getShortTimeFormatAccess().getGroup(), "rule__ShortTimeFormat__Group__0");
					put(grammarAccess.getSortAccess().getGroup(), "rule__Sort__Group__0");
					put(grammarAccess.getSortAccess().getGroup_2(), "rule__Sort__Group_2__0");
					put(grammarAccess.getSortAccountsAccess().getGroup(), "rule__SortAccounts__Group__0");
					put(grammarAccess.getSortJournalEntriesAccess().getGroup(), "rule__SortJournalEntries__Group__0");
					put(grammarAccess.getSortResourcesAccess().getGroup(), "rule__SortResources__Group__0");
					put(grammarAccess.getSortTasksAccess().getGroup(), "rule__SortTasks__Group__0");
					put(grammarAccess.getStartAccess().getGroup(), "rule__Start__Group__0");
					put(grammarAccess.getStatusStatusSheetAccess().getGroup(), "rule__StatusStatusSheet__Group__0");
					put(grammarAccess.getStatusStatusSheetAccess().getGroup_3(), "rule__StatusStatusSheet__Group_3__0");
					put(grammarAccess.getStatusTimesheetAccess().getGroup(), "rule__StatusTimesheet__Group__0");
					put(grammarAccess.getStatusTimesheetAccess().getGroup_3(), "rule__StatusTimesheet__Group_3__0");
					put(grammarAccess.getStatusSheetAccess().getGroup(), "rule__StatusSheet__Group__0");
					put(grammarAccess.getStatusSheetAccess().getGroup_3(), "rule__StatusSheet__Group_3__0");
					put(grammarAccess.getStatusSheetReportAccess().getGroup(), "rule__StatusSheetReport__Group__0");
					put(grammarAccess.getStatusSheetReportAccess().getGroup_2(), "rule__StatusSheetReport__Group_2__0");
					put(grammarAccess.getSummaryAccess().getGroup(), "rule__Summary__Group__0");
					put(grammarAccess.getSupplementAccountAccess().getGroup(), "rule__SupplementAccount__Group__0");
					put(grammarAccess.getSupplementAccountAccess().getGroup_3(), "rule__SupplementAccount__Group_3__0");
					put(grammarAccess.getSupplementReportAccess().getGroup(), "rule__SupplementReport__Group__0");
					put(grammarAccess.getSupplementReportAccess().getGroup_3(), "rule__SupplementReport__Group_3__0");
					put(grammarAccess.getSupplementResourceAccess().getGroup(), "rule__SupplementResource__Group__0");
					put(grammarAccess.getSupplementResourceAccess().getGroup_3(), "rule__SupplementResource__Group_3__0");
					put(grammarAccess.getSupplementTaskAccess().getGroup(), "rule__SupplementTask__Group__0");
					put(grammarAccess.getSupplementTaskAccess().getGroup_3(), "rule__SupplementTask__Group_3__0");
					put(grammarAccess.getTagFileAccess().getGroup(), "rule__TagFile__Group__0");
					put(grammarAccess.getTagFileAccess().getGroup_3_0(), "rule__TagFile__Group_3_0__0");
					put(grammarAccess.getTagFileAccess().getGroup_3_3(), "rule__TagFile__Group_3_3__0");
					put(grammarAccess.getTaskStatusSheetAccess().getGroup(), "rule__TaskStatusSheet__Group__0");
					put(grammarAccess.getTaskStatusSheetAccess().getGroup_2(), "rule__TaskStatusSheet__Group_2__0");
					put(grammarAccess.getTaskTimesheetAccess().getGroup(), "rule__TaskTimesheet__Group__0");
					put(grammarAccess.getTaskTimesheetAccess().getGroup_2(), "rule__TaskTimesheet__Group_2__0");
					put(grammarAccess.getTaskAttributesAccess().getGroup(), "rule__TaskAttributes__Group__0");
					put(grammarAccess.getTaskAttributesAccess().getGroup_1_2(), "rule__TaskAttributes__Group_1_2__0");
					put(grammarAccess.getTaskPrefixAccess().getGroup(), "rule__TaskPrefix__Group__0");
					put(grammarAccess.getTaskReportAccess().getGroup(), "rule__TaskReport__Group__0");
					put(grammarAccess.getTaskRootAccess().getGroup(), "rule__TaskRoot__Group__0");
					put(grammarAccess.getTextReportAccess().getGroup(), "rule__TextReport__Group__0");
					put(grammarAccess.getTimeFormatAccess().getGroup(), "rule__TimeFormat__Group__0");
					put(grammarAccess.getTimeoffAccess().getGroup(), "rule__Timeoff__Group__0");
					put(grammarAccess.getTimesheetAccess().getGroup(), "rule__Timesheet__Group__0");
					put(grammarAccess.getTimesheetAccess().getGroup_3(), "rule__Timesheet__Group_3__0");
					put(grammarAccess.getTimesheetReportAccess().getGroup(), "rule__TimesheetReport__Group__0");
					put(grammarAccess.getTimesheetReportAccess().getGroup_2(), "rule__TimesheetReport__Group_2__0");
					put(grammarAccess.getTimezoneAccess().getGroup(), "rule__Timezone__Group__0");
					put(grammarAccess.getTimingResolutionAccess().getGroup(), "rule__TimingResolution__Group__0");
					put(grammarAccess.getTitleAccess().getGroup(), "rule__Title__Group__0");
					put(grammarAccess.getToolTipAccess().getGroup(), "rule__ToolTip__Group__0");
					put(grammarAccess.getTrackingScenarioAccess().getGroup(), "rule__TrackingScenario__Group__0");
					put(grammarAccess.getTreeLevelAccess().getGroup(), "rule__TreeLevel__Group__0");
					put(grammarAccess.getVacationAccess().getGroup(), "rule__Vacation__Group__0");
					put(grammarAccess.getVacationAccess().getGroup_3(), "rule__Vacation__Group_3__0");
					put(grammarAccess.getVacationShiftAccess().getGroup(), "rule__VacationShift__Group__0");
					put(grammarAccess.getVacationShiftAccess().getGroup_3(), "rule__VacationShift__Group_3__0");
					put(grammarAccess.getVacationShiftAccess().getGroup_3_1(), "rule__VacationShift__Group_3_1__0");
					put(grammarAccess.getVacationResourceAccess().getGroup(), "rule__VacationResource__Group__0");
					put(grammarAccess.getVacationResourceAccess().getGroup_3(), "rule__VacationResource__Group_3__0");
					put(grammarAccess.getWarnAccess().getGroup(), "rule__Warn__Group__0");
					put(grammarAccess.getWeeklyMaxAccess().getGroup(), "rule__WeeklyMax__Group__0");
					put(grammarAccess.getWeeklyMinAccess().getGroup(), "rule__WeeklyMin__Group__0");
					put(grammarAccess.getWidthAccess().getGroup(), "rule__Width__Group__0");
					put(grammarAccess.getWorkAccess().getGroup(), "rule__Work__Group__0");
					put(grammarAccess.getWorkingHoursAccess().getGroup(), "rule__WorkingHours__Group__0");
					put(grammarAccess.getWorkingHoursAccess().getGroup_1(), "rule__WorkingHours__Group_1__0");
					put(grammarAccess.getWorkingHoursAccess().getGroup_1_1(), "rule__WorkingHours__Group_1_1__0");
					put(grammarAccess.getWorkingHoursAccess().getGroup_2_1(), "rule__WorkingHours__Group_2_1__0");
					put(grammarAccess.getWorkingHoursAccess().getGroup_2_1_1(), "rule__WorkingHours__Group_2_1_1__0");
					put(grammarAccess.getYearlyWorkingDaysAccess().getGroup(), "rule__YearlyWorkingDays__Group__0");
					put(grammarAccess.getAccountShareAccess().getGroup(), "rule__AccountShare__Group__0");
					put(grammarAccess.getColumnAccess().getGroup(), "rule__Column__Group__0");
					put(grammarAccess.getCriterionAccess().getGroup(), "rule__Criterion__Group__0");
					put(grammarAccess.getDurationQuantityAccess().getGroup(), "rule__DurationQuantity__Group__0");
					put(grammarAccess.getLimitAccess().getGroup(), "rule__Limit__Group__0");
					put(grammarAccess.getLimitAccess().getGroup_1(), "rule__Limit__Group_1__0");
					put(grammarAccess.getLimitAttributeAccess().getGroup_0(), "rule__LimitAttribute__Group_0__0");
					put(grammarAccess.getLimitAttributeAccess().getGroup_1(), "rule__LimitAttribute__Group_1__0");
					put(grammarAccess.getLimitAttributeAccess().getGroup_2(), "rule__LimitAttribute__Group_2__0");
					put(grammarAccess.getLimitAttributeAccess().getGroup_2_2(), "rule__LimitAttribute__Group_2_2__0");
					put(grammarAccess.getLimitAttributeAccess().getGroup_3(), "rule__LimitAttribute__Group_3__0");
					put(grammarAccess.getRealFormatAccess().getGroup(), "rule__RealFormat__Group__0");
					put(grammarAccess.getTaskDependencyAccess().getGroup(), "rule__TaskDependency__Group__0");
					put(grammarAccess.getTaskDependencyAccess().getGroup_1(), "rule__TaskDependency__Group_1__0");
					put(grammarAccess.getTaskPathAccess().getGroup(), "rule__TaskPath__Group__0");
					put(grammarAccess.getTaskPathAccess().getGroup_2(), "rule__TaskPath__Group_2__0");
					put(grammarAccess.getWorkHoursAccess().getGroup(), "rule__WorkHours__Group__0");
					put(grammarAccess.getWeekdaysAccess().getGroup(), "rule__Weekdays__Group__0");
					put(grammarAccess.getWeekdaysAccess().getGroup_1(), "rule__Weekdays__Group_1__0");
					put(grammarAccess.getXAssignmentAccess().getGroup_0(), "rule__XAssignment__Group_0__0");
					put(grammarAccess.getXAssignmentAccess().getGroup_1(), "rule__XAssignment__Group_1__0");
					put(grammarAccess.getXAssignmentAccess().getGroup_1_1(), "rule__XAssignment__Group_1_1__0");
					put(grammarAccess.getXAssignmentAccess().getGroup_1_1_0(), "rule__XAssignment__Group_1_1_0__0");
					put(grammarAccess.getXAssignmentAccess().getGroup_1_1_0_0(), "rule__XAssignment__Group_1_1_0_0__0");
					put(grammarAccess.getXOrExpressionAccess().getGroup(), "rule__XOrExpression__Group__0");
					put(grammarAccess.getXOrExpressionAccess().getGroup_1(), "rule__XOrExpression__Group_1__0");
					put(grammarAccess.getXOrExpressionAccess().getGroup_1_0(), "rule__XOrExpression__Group_1_0__0");
					put(grammarAccess.getXOrExpressionAccess().getGroup_1_0_0(), "rule__XOrExpression__Group_1_0_0__0");
					put(grammarAccess.getXAndExpressionAccess().getGroup(), "rule__XAndExpression__Group__0");
					put(grammarAccess.getXAndExpressionAccess().getGroup_1(), "rule__XAndExpression__Group_1__0");
					put(grammarAccess.getXAndExpressionAccess().getGroup_1_0(), "rule__XAndExpression__Group_1_0__0");
					put(grammarAccess.getXAndExpressionAccess().getGroup_1_0_0(), "rule__XAndExpression__Group_1_0_0__0");
					put(grammarAccess.getXEqualityExpressionAccess().getGroup(), "rule__XEqualityExpression__Group__0");
					put(grammarAccess.getXEqualityExpressionAccess().getGroup_1(), "rule__XEqualityExpression__Group_1__0");
					put(grammarAccess.getXEqualityExpressionAccess().getGroup_1_0(), "rule__XEqualityExpression__Group_1_0__0");
					put(grammarAccess.getXEqualityExpressionAccess().getGroup_1_0_0(), "rule__XEqualityExpression__Group_1_0_0__0");
					put(grammarAccess.getXRelationalExpressionAccess().getGroup(), "rule__XRelationalExpression__Group__0");
					put(grammarAccess.getXRelationalExpressionAccess().getGroup_1_0(), "rule__XRelationalExpression__Group_1_0__0");
					put(grammarAccess.getXRelationalExpressionAccess().getGroup_1_0_0(), "rule__XRelationalExpression__Group_1_0_0__0");
					put(grammarAccess.getXRelationalExpressionAccess().getGroup_1_0_0_0(), "rule__XRelationalExpression__Group_1_0_0_0__0");
					put(grammarAccess.getXRelationalExpressionAccess().getGroup_1_1(), "rule__XRelationalExpression__Group_1_1__0");
					put(grammarAccess.getXRelationalExpressionAccess().getGroup_1_1_0(), "rule__XRelationalExpression__Group_1_1_0__0");
					put(grammarAccess.getXRelationalExpressionAccess().getGroup_1_1_0_0(), "rule__XRelationalExpression__Group_1_1_0_0__0");
					put(grammarAccess.getXOtherOperatorExpressionAccess().getGroup(), "rule__XOtherOperatorExpression__Group__0");
					put(grammarAccess.getXOtherOperatorExpressionAccess().getGroup_1(), "rule__XOtherOperatorExpression__Group_1__0");
					put(grammarAccess.getXOtherOperatorExpressionAccess().getGroup_1_0(), "rule__XOtherOperatorExpression__Group_1_0__0");
					put(grammarAccess.getXOtherOperatorExpressionAccess().getGroup_1_0_0(), "rule__XOtherOperatorExpression__Group_1_0_0__0");
					put(grammarAccess.getXAdditiveExpressionAccess().getGroup(), "rule__XAdditiveExpression__Group__0");
					put(grammarAccess.getXAdditiveExpressionAccess().getGroup_1(), "rule__XAdditiveExpression__Group_1__0");
					put(grammarAccess.getXAdditiveExpressionAccess().getGroup_1_0(), "rule__XAdditiveExpression__Group_1_0__0");
					put(grammarAccess.getXAdditiveExpressionAccess().getGroup_1_0_0(), "rule__XAdditiveExpression__Group_1_0_0__0");
					put(grammarAccess.getXMultiplicativeExpressionAccess().getGroup(), "rule__XMultiplicativeExpression__Group__0");
					put(grammarAccess.getXMultiplicativeExpressionAccess().getGroup_1(), "rule__XMultiplicativeExpression__Group_1__0");
					put(grammarAccess.getXMultiplicativeExpressionAccess().getGroup_1_0(), "rule__XMultiplicativeExpression__Group_1_0__0");
					put(grammarAccess.getXMultiplicativeExpressionAccess().getGroup_1_0_0(), "rule__XMultiplicativeExpression__Group_1_0_0__0");
					put(grammarAccess.getXUnaryOperationAccess().getGroup_0(), "rule__XUnaryOperation__Group_0__0");
					put(grammarAccess.getXCastedExpressionAccess().getGroup(), "rule__XCastedExpression__Group__0");
					put(grammarAccess.getXCastedExpressionAccess().getGroup_1(), "rule__XCastedExpression__Group_1__0");
					put(grammarAccess.getXCastedExpressionAccess().getGroup_1_0(), "rule__XCastedExpression__Group_1_0__0");
					put(grammarAccess.getXCastedExpressionAccess().getGroup_1_0_0(), "rule__XCastedExpression__Group_1_0_0__0");
					put(grammarAccess.getXMemberFeatureCallAccess().getGroup(), "rule__XMemberFeatureCall__Group__0");
					put(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_0(), "rule__XMemberFeatureCall__Group_1_0__0");
					put(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_0_0(), "rule__XMemberFeatureCall__Group_1_0_0__0");
					put(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_0_0_0(), "rule__XMemberFeatureCall__Group_1_0_0_0__0");
					put(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_1(), "rule__XMemberFeatureCall__Group_1_1__0");
					put(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_1_0(), "rule__XMemberFeatureCall__Group_1_1_0__0");
					put(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_1_0_0(), "rule__XMemberFeatureCall__Group_1_1_0_0__0");
					put(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_1_1(), "rule__XMemberFeatureCall__Group_1_1_1__0");
					put(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_1_1_2(), "rule__XMemberFeatureCall__Group_1_1_1_2__0");
					put(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_1_3(), "rule__XMemberFeatureCall__Group_1_1_3__0");
					put(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_1_3_1_1(), "rule__XMemberFeatureCall__Group_1_1_3_1_1__0");
					put(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_1_3_1_1_1(), "rule__XMemberFeatureCall__Group_1_1_3_1_1_1__0");
					put(grammarAccess.getXClosureAccess().getGroup(), "rule__XClosure__Group__0");
					put(grammarAccess.getXClosureAccess().getGroup_2(), "rule__XClosure__Group_2__0");
					put(grammarAccess.getXClosureAccess().getGroup_2_0(), "rule__XClosure__Group_2_0__0");
					put(grammarAccess.getXClosureAccess().getGroup_2_0_0(), "rule__XClosure__Group_2_0_0__0");
					put(grammarAccess.getXClosureAccess().getGroup_2_0_0_1(), "rule__XClosure__Group_2_0_0_1__0");
					put(grammarAccess.getXExpressionInClosureAccess().getGroup(), "rule__XExpressionInClosure__Group__0");
					put(grammarAccess.getXExpressionInClosureAccess().getGroup_1(), "rule__XExpressionInClosure__Group_1__0");
					put(grammarAccess.getXShortClosureAccess().getGroup(), "rule__XShortClosure__Group__0");
					put(grammarAccess.getXShortClosureAccess().getGroup_0(), "rule__XShortClosure__Group_0__0");
					put(grammarAccess.getXShortClosureAccess().getGroup_0_0(), "rule__XShortClosure__Group_0_0__0");
					put(grammarAccess.getXShortClosureAccess().getGroup_0_0_1(), "rule__XShortClosure__Group_0_0_1__0");
					put(grammarAccess.getXShortClosureAccess().getGroup_0_0_1_1(), "rule__XShortClosure__Group_0_0_1_1__0");
					put(grammarAccess.getXParenthesizedExpressionAccess().getGroup(), "rule__XParenthesizedExpression__Group__0");
					put(grammarAccess.getXIfExpressionAccess().getGroup(), "rule__XIfExpression__Group__0");
					put(grammarAccess.getXIfExpressionAccess().getGroup_6(), "rule__XIfExpression__Group_6__0");
					put(grammarAccess.getXSwitchExpressionAccess().getGroup(), "rule__XSwitchExpression__Group__0");
					put(grammarAccess.getXSwitchExpressionAccess().getGroup_2(), "rule__XSwitchExpression__Group_2__0");
					put(grammarAccess.getXSwitchExpressionAccess().getGroup_2_0(), "rule__XSwitchExpression__Group_2_0__0");
					put(grammarAccess.getXSwitchExpressionAccess().getGroup_6(), "rule__XSwitchExpression__Group_6__0");
					put(grammarAccess.getXCasePartAccess().getGroup(), "rule__XCasePart__Group__0");
					put(grammarAccess.getXCasePartAccess().getGroup_1(), "rule__XCasePart__Group_1__0");
					put(grammarAccess.getXForLoopExpressionAccess().getGroup(), "rule__XForLoopExpression__Group__0");
					put(grammarAccess.getXWhileExpressionAccess().getGroup(), "rule__XWhileExpression__Group__0");
					put(grammarAccess.getXDoWhileExpressionAccess().getGroup(), "rule__XDoWhileExpression__Group__0");
					put(grammarAccess.getXBlockExpressionAccess().getGroup(), "rule__XBlockExpression__Group__0");
					put(grammarAccess.getXBlockExpressionAccess().getGroup_2(), "rule__XBlockExpression__Group_2__0");
					put(grammarAccess.getXVariableDeclarationAccess().getGroup(), "rule__XVariableDeclaration__Group__0");
					put(grammarAccess.getXVariableDeclarationAccess().getGroup_2_0(), "rule__XVariableDeclaration__Group_2_0__0");
					put(grammarAccess.getXVariableDeclarationAccess().getGroup_2_0_0(), "rule__XVariableDeclaration__Group_2_0_0__0");
					put(grammarAccess.getXVariableDeclarationAccess().getGroup_3(), "rule__XVariableDeclaration__Group_3__0");
					put(grammarAccess.getJvmFormalParameterAccess().getGroup(), "rule__JvmFormalParameter__Group__0");
					put(grammarAccess.getFullJvmFormalParameterAccess().getGroup(), "rule__FullJvmFormalParameter__Group__0");
					put(grammarAccess.getXFeatureCallAccess().getGroup(), "rule__XFeatureCall__Group__0");
					put(grammarAccess.getXFeatureCallAccess().getGroup_2(), "rule__XFeatureCall__Group_2__0");
					put(grammarAccess.getXFeatureCallAccess().getGroup_2_2(), "rule__XFeatureCall__Group_2_2__0");
					put(grammarAccess.getXFeatureCallAccess().getGroup_4(), "rule__XFeatureCall__Group_4__0");
					put(grammarAccess.getXFeatureCallAccess().getGroup_4_1_1(), "rule__XFeatureCall__Group_4_1_1__0");
					put(grammarAccess.getXFeatureCallAccess().getGroup_4_1_1_1(), "rule__XFeatureCall__Group_4_1_1_1__0");
					put(grammarAccess.getStaticQualifierAccess().getGroup(), "rule__StaticQualifier__Group__0");
					put(grammarAccess.getXConstructorCallAccess().getGroup(), "rule__XConstructorCall__Group__0");
					put(grammarAccess.getXConstructorCallAccess().getGroup_3(), "rule__XConstructorCall__Group_3__0");
					put(grammarAccess.getXConstructorCallAccess().getGroup_3_2(), "rule__XConstructorCall__Group_3_2__0");
					put(grammarAccess.getXConstructorCallAccess().getGroup_5_1(), "rule__XConstructorCall__Group_5_1__0");
					put(grammarAccess.getXConstructorCallAccess().getGroup_5_1_1(), "rule__XConstructorCall__Group_5_1_1__0");
					put(grammarAccess.getXBooleanLiteralAccess().getGroup(), "rule__XBooleanLiteral__Group__0");
					put(grammarAccess.getXNullLiteralAccess().getGroup(), "rule__XNullLiteral__Group__0");
					put(grammarAccess.getXIntLiteralAccess().getGroup(), "rule__XIntLiteral__Group__0");
					put(grammarAccess.getXStringLiteralAccess().getGroup(), "rule__XStringLiteral__Group__0");
					put(grammarAccess.getXTypeLiteralAccess().getGroup(), "rule__XTypeLiteral__Group__0");
					put(grammarAccess.getXThrowExpressionAccess().getGroup(), "rule__XThrowExpression__Group__0");
					put(grammarAccess.getXReturnExpressionAccess().getGroup(), "rule__XReturnExpression__Group__0");
					put(grammarAccess.getXTryCatchFinallyExpressionAccess().getGroup(), "rule__XTryCatchFinallyExpression__Group__0");
					put(grammarAccess.getXTryCatchFinallyExpressionAccess().getGroup_3_0(), "rule__XTryCatchFinallyExpression__Group_3_0__0");
					put(grammarAccess.getXTryCatchFinallyExpressionAccess().getGroup_3_0_1(), "rule__XTryCatchFinallyExpression__Group_3_0_1__0");
					put(grammarAccess.getXTryCatchFinallyExpressionAccess().getGroup_3_1(), "rule__XTryCatchFinallyExpression__Group_3_1__0");
					put(grammarAccess.getXCatchClauseAccess().getGroup(), "rule__XCatchClause__Group__0");
					put(grammarAccess.getQualifiedNameAccess().getGroup(), "rule__QualifiedName__Group__0");
					put(grammarAccess.getQualifiedNameAccess().getGroup_1(), "rule__QualifiedName__Group_1__0");
					put(grammarAccess.getJvmTypeReferenceAccess().getGroup_0(), "rule__JvmTypeReference__Group_0__0");
					put(grammarAccess.getJvmTypeReferenceAccess().getGroup_0_1(), "rule__JvmTypeReference__Group_0_1__0");
					put(grammarAccess.getJvmTypeReferenceAccess().getGroup_0_1_0(), "rule__JvmTypeReference__Group_0_1_0__0");
					put(grammarAccess.getXFunctionTypeRefAccess().getGroup(), "rule__XFunctionTypeRef__Group__0");
					put(grammarAccess.getXFunctionTypeRefAccess().getGroup_0(), "rule__XFunctionTypeRef__Group_0__0");
					put(grammarAccess.getXFunctionTypeRefAccess().getGroup_0_1(), "rule__XFunctionTypeRef__Group_0_1__0");
					put(grammarAccess.getXFunctionTypeRefAccess().getGroup_0_1_1(), "rule__XFunctionTypeRef__Group_0_1_1__0");
					put(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGroup(), "rule__JvmParameterizedTypeReference__Group__0");
					put(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGroup_1(), "rule__JvmParameterizedTypeReference__Group_1__0");
					put(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGroup_1_2(), "rule__JvmParameterizedTypeReference__Group_1_2__0");
					put(grammarAccess.getJvmWildcardTypeReferenceAccess().getGroup(), "rule__JvmWildcardTypeReference__Group__0");
					put(grammarAccess.getJvmUpperBoundAccess().getGroup(), "rule__JvmUpperBound__Group__0");
					put(grammarAccess.getJvmUpperBoundAndedAccess().getGroup(), "rule__JvmUpperBoundAnded__Group__0");
					put(grammarAccess.getJvmLowerBoundAccess().getGroup(), "rule__JvmLowerBound__Group__0");
					put(grammarAccess.getJvmTypeParameterAccess().getGroup(), "rule__JvmTypeParameter__Group__0");
					put(grammarAccess.getJvmTypeParameterAccess().getGroup_1_0(), "rule__JvmTypeParameter__Group_1_0__0");
					put(grammarAccess.getGlobalAccess().getProjectAssignment_0(), "rule__Global__ProjectAssignment_0");
					put(grammarAccess.getGlobalAccess().getPropertiesAssignment_1(), "rule__Global__PropertiesAssignment_1");
					put(grammarAccess.getAccountAccess().getIdAssignment_1(), "rule__Account__IdAssignment_1");
					put(grammarAccess.getAccountAccess().getNameAssignment_2(), "rule__Account__NameAssignment_2");
					put(grammarAccess.getAccountAccess().getAttributesAssignment_3_1(), "rule__Account__AttributesAssignment_3_1");
					put(grammarAccess.getAccountPrefixAccess().getAccountAssignment_1(), "rule__AccountPrefix__AccountAssignment_1");
					put(grammarAccess.getAccountRootAccess().getAccountAssignment_1(), "rule__AccountRoot__AccountAssignment_1");
					put(grammarAccess.getProjectAccess().getIdAssignment_1(), "rule__Project__IdAssignment_1");
					put(grammarAccess.getProjectAccess().getNameAssignment_2(), "rule__Project__NameAssignment_2");
					put(grammarAccess.getProjectAccess().getVersionAssignment_3(), "rule__Project__VersionAssignment_3");
					put(grammarAccess.getProjectAccess().getIntervalAssignment_4(), "rule__Project__IntervalAssignment_4");
					put(grammarAccess.getProjectAccess().getAttributesAssignment_5_1(), "rule__Project__AttributesAssignment_5_1");
					put(grammarAccess.getTaskAccess().getIdAssignment_1(), "rule__Task__IdAssignment_1");
					put(grammarAccess.getTaskAccess().getNameAssignment_2(), "rule__Task__NameAssignment_2");
					put(grammarAccess.getTaskAccess().getAttributesAssignment_3_1(), "rule__Task__AttributesAssignment_3_1");
					put(grammarAccess.getReportAccess().getIdAssignment_0(), "rule__Report__IdAssignment_0");
					put(grammarAccess.getReportAccess().getNameAssignment_1(), "rule__Report__NameAssignment_1");
					put(grammarAccess.getReportAccess().getAttributesAssignment_2_1(), "rule__Report__AttributesAssignment_2_1");
					put(grammarAccess.getIcalReportAccess().getFilenameAssignment_1(), "rule__IcalReport__FilenameAssignment_1");
					put(grammarAccess.getIcalReportAccess().getAttributesAssignment_2_1(), "rule__IcalReport__AttributesAssignment_2_1");
					put(grammarAccess.getExportAccess().getIdAssignment_1(), "rule__Export__IdAssignment_1");
					put(grammarAccess.getExportAccess().getFilenameAssignment_2(), "rule__Export__FilenameAssignment_2");
					put(grammarAccess.getExportAccess().getAttributesAssignment_3_1(), "rule__Export__AttributesAssignment_3_1");
					put(grammarAccess.getResourceAccess().getIdAssignment_1(), "rule__Resource__IdAssignment_1");
					put(grammarAccess.getResourceAccess().getNameAssignment_2(), "rule__Resource__NameAssignment_2");
					put(grammarAccess.getResourceAccess().getAttributesAssignment_3_1(), "rule__Resource__AttributesAssignment_3_1");
					put(grammarAccess.getAllocateAccess().getResourcesAssignment_1(), "rule__Allocate__ResourcesAssignment_1");
					put(grammarAccess.getAllocateAccess().getResourcesAssignment_2_1(), "rule__Allocate__ResourcesAssignment_2_1");
					put(grammarAccess.getAllocateResourceAccess().getResourceAssignment_0(), "rule__AllocateResource__ResourceAssignment_0");
					put(grammarAccess.getAllocateResourceAccess().getAttributesAssignment_1_1(), "rule__AllocateResource__AttributesAssignment_1_1");
					put(grammarAccess.getNavigatorAccess().getIdAssignment_1(), "rule__Navigator__IdAssignment_1");
					put(grammarAccess.getNavigatorAccess().getAttributesAssignment_2_1(), "rule__Navigator__AttributesAssignment_2_1");
					put(grammarAccess.getNewTaskAccess().getIdAssignment_1(), "rule__NewTask__IdAssignment_1");
					put(grammarAccess.getNewTaskAccess().getTextAssignment_2(), "rule__NewTask__TextAssignment_2");
					put(grammarAccess.getNewTaskAccess().getAttributesAssignment_3_1(), "rule__NewTask__AttributesAssignment_3_1");
					put(grammarAccess.getNikuReportAccess().getFilenameAssignment_1(), "rule__NikuReport__FilenameAssignment_1");
					put(grammarAccess.getNikuReportAccess().getAttributesAssignment_2_1(), "rule__NikuReport__AttributesAssignment_2_1");
					put(grammarAccess.getAlertAccess().getLevelAssignment_1(), "rule__Alert__LevelAssignment_1");
					put(grammarAccess.getAlternativeAccess().getResourcesAssignment_0(), "rule__Alternative__ResourcesAssignment_0");
					put(grammarAccess.getAlternativeAccess().getResourcesAssignment_1_1(), "rule__Alternative__ResourcesAssignment_1_1");
					put(grammarAccess.getAuthorAccess().getResourceAssignment_1(), "rule__Author__ResourceAssignment_1");
					put(grammarAccess.getBalanceAccess().getCostAssignment_1(), "rule__Balance__CostAssignment_1");
					put(grammarAccess.getBalanceAccess().getRevenueAssignment_2(), "rule__Balance__RevenueAssignment_2");
					put(grammarAccess.getBookingAccess().getIntervalAssignment_0(), "rule__Booking__IntervalAssignment_0");
					put(grammarAccess.getBookingAccess().getOvertimeAssignment_1_1_0_1(), "rule__Booking__OvertimeAssignment_1_1_0_1");
					put(grammarAccess.getBookingAccess().getSloppyAssignment_1_1_1_1(), "rule__Booking__SloppyAssignment_1_1_1_1");
					put(grammarAccess.getBookingTaskAccess().getResourceAssignment_1(), "rule__BookingTask__ResourceAssignment_1");
					put(grammarAccess.getBookingTaskAccess().getBookingAssignment_2(), "rule__BookingTask__BookingAssignment_2");
					put(grammarAccess.getBookingResourceAccess().getTaskAssignment_1(), "rule__BookingResource__TaskAssignment_1");
					put(grammarAccess.getBookingResourceAccess().getBookingAssignment_2(), "rule__BookingResource__BookingAssignment_2");
					put(grammarAccess.getCellColorAccess().getExpressionAssignment_1(), "rule__CellColor__ExpressionAssignment_1");
					put(grammarAccess.getCellColorAccess().getColorAssignment_2(), "rule__CellColor__ColorAssignment_2");
					put(grammarAccess.getCellTextAccess().getTextAssignment_1(), "rule__CellText__TextAssignment_1");
					put(grammarAccess.getCellTextAccess().getExpresssionAssignment_2(), "rule__CellText__ExpresssionAssignment_2");
					put(grammarAccess.getChargeAccess().getAmountAssignment_1(), "rule__Charge__AmountAssignment_1");
					put(grammarAccess.getChargeAccess().getAppliesAssignment_2(), "rule__Charge__AppliesAssignment_2");
					put(grammarAccess.getChargeSetAccess().getAccountSharesAssignment_1(), "rule__ChargeSet__AccountSharesAssignment_1");
					put(grammarAccess.getChargeSetAccess().getAccountSharesAssignment_2_1(), "rule__ChargeSet__AccountSharesAssignment_2_1");
					put(grammarAccess.getColumnsAccess().getColumnsAssignment_1(), "rule__Columns__ColumnsAssignment_1");
					put(grammarAccess.getColumnsAccess().getColumnsAssignment_2_1(), "rule__Columns__ColumnsAssignment_2_1");
					put(grammarAccess.getCompleteAccess().getCompleteAssignment_1(), "rule__Complete__CompleteAssignment_1");
					put(grammarAccess.getCopyrightAccess().getTextAssignment_1(), "rule__Copyright__TextAssignment_1");
					put(grammarAccess.getCreditAccess().getDateAssignment_1(), "rule__Credit__DateAssignment_1");
					put(grammarAccess.getCreditAccess().getDescriptionAssignment_2(), "rule__Credit__DescriptionAssignment_2");
					put(grammarAccess.getCreditAccess().getAmountAssignment_3(), "rule__Credit__AmountAssignment_3");
					put(grammarAccess.getCurrencyAccess().getCurrencyAssignment_1(), "rule__Currency__CurrencyAssignment_1");
					put(grammarAccess.getDailyWorkingHoursAccess().getDailyWorkingHoursAssignment_1(), "rule__DailyWorkingHours__DailyWorkingHoursAssignment_1");
					put(grammarAccess.getDefinitionsAccess().getAllAssignment_1_0(), "rule__Definitions__AllAssignment_1_0");
					put(grammarAccess.getDefinitionsAccess().getNoneAssignment_1_1(), "rule__Definitions__NoneAssignment_1_1");
					put(grammarAccess.getDefinitionsAccess().getFlagsAssignment_1_2_1_0(), "rule__Definitions__FlagsAssignment_1_2_1_0");
					put(grammarAccess.getDefinitionsAccess().getResourcesAssignment_1_2_1_1(), "rule__Definitions__ResourcesAssignment_1_2_1_1");
					put(grammarAccess.getDefinitionsAccess().getTasksAssignment_1_2_1_2(), "rule__Definitions__TasksAssignment_1_2_1_2");
					put(grammarAccess.getDefinitionsAccess().getProjectAssignment_1_2_1_3(), "rule__Definitions__ProjectAssignment_1_2_1_3");
					put(grammarAccess.getDefinitionsAccess().getProjectidsAssignment_1_2_1_4(), "rule__Definitions__ProjectidsAssignment_1_2_1_4");
					put(grammarAccess.getDurationAccess().getDurationAssignment_1(), "rule__Duration__DurationAssignment_1");
					put(grammarAccess.getEfficiencyAccess().getEfficiencyAssignment_1(), "rule__Efficiency__EfficiencyAssignment_1");
					put(grammarAccess.getEffortAccess().getEffortAssignment_1(), "rule__Effort__EffortAssignment_1");
					put(grammarAccess.getEmailAccess().getAddressAssignment_1(), "rule__Email__AddressAssignment_1");
					put(grammarAccess.getEndAccess().getEndAssignment_1(), "rule__End__EndAssignment_1");
					put(grammarAccess.getEndCreditAccess().getCreditAssignment_1(), "rule__EndCredit__CreditAssignment_1");
					put(grammarAccess.getExtendAccess().getIdAssignment_1(), "rule__Extend__IdAssignment_1");
					put(grammarAccess.getExtendAccess().getNameAssignment_2(), "rule__Extend__NameAssignment_2");
					put(grammarAccess.getExtendAccess().getInheritAssignment_3_1_0(), "rule__Extend__InheritAssignment_3_1_0");
					put(grammarAccess.getExtendAccess().getScenariospecificAssignment_3_1_1(), "rule__Extend__ScenariospecificAssignment_3_1_1");
					put(grammarAccess.getExtendResourceAccess().getExtendsAssignment_3_1(), "rule__ExtendResource__ExtendsAssignment_3_1");
					put(grammarAccess.getExtendedResourceAttributeAccess().getExtendAssignment_0(), "rule__ExtendedResourceAttribute__ExtendAssignment_0");
					put(grammarAccess.getExtendedResourceAttributeAccess().getValueAssignment_1(), "rule__ExtendedResourceAttribute__ValueAssignment_1");
					put(grammarAccess.getExtendTaskAccess().getExtendsAssignment_3_1(), "rule__ExtendTask__ExtendsAssignment_3_1");
					put(grammarAccess.getExtendedTaskAttributeAccess().getExtendAssignment_0(), "rule__ExtendedTaskAttribute__ExtendAssignment_0");
					put(grammarAccess.getExtendedTaskAttributeAccess().getValueAssignment_1(), "rule__ExtendedTaskAttribute__ValueAssignment_1");
					put(grammarAccess.getFailAccess().getExpressionAssignment_1(), "rule__Fail__ExpressionAssignment_1");
					put(grammarAccess.getFlagsAccess().getFlagsAssignment_1(), "rule__Flags__FlagsAssignment_1");
					put(grammarAccess.getFlagsAccess().getFlagsAssignment_2_1(), "rule__Flags__FlagsAssignment_2_1");
					put(grammarAccess.getFontColorAccess().getColorAssignment_1(), "rule__FontColor__ColorAssignment_1");
					put(grammarAccess.getFormatsAccess().getFormatsAssignment_1(), "rule__Formats__FormatsAssignment_1");
					put(grammarAccess.getFormatsAccess().getFormatsAssignment_2_1(), "rule__Formats__FormatsAssignment_2_1");
					put(grammarAccess.getFunctionAccess().getLevelAssignment_1_0_2(), "rule__Function__LevelAssignment_1_0_2");
					put(grammarAccess.getFunctionAccess().getDateAssignment_1_0_4(), "rule__Function__DateAssignment_1_0_4");
					put(grammarAccess.getFunctionAccess().getScenarioAssignment_1_1_2(), "rule__Function__ScenarioAssignment_1_1_2");
					put(grammarAccess.getFunctionAccess().getParentIdAssignment_1_2_2(), "rule__Function__ParentIdAssignment_1_2_2");
					put(grammarAccess.getFunctionAccess().getTaskAssignment_1_3_2(), "rule__Function__TaskAssignment_1_3_2");
					put(grammarAccess.getFunctionAccess().getScenarioAssignment_1_3_4(), "rule__Function__ScenarioAssignment_1_3_4");
					put(grammarAccess.getFunctionAccess().getDistanceAssignment_1_3_6(), "rule__Function__DistanceAssignment_1_3_6");
					put(grammarAccess.getFunctionAccess().getResourceAssignment_1_4_2(), "rule__Function__ResourceAssignment_1_4_2");
					put(grammarAccess.getFunctionAccess().getScenarioAssignment_1_4_4(), "rule__Function__ScenarioAssignment_1_4_4");
					put(grammarAccess.getFunctionAccess().getTaskAssignment_1_5_2(), "rule__Function__TaskAssignment_1_5_2");
					put(grammarAccess.getFunctionAccess().getScenarioAssignment_1_5_4(), "rule__Function__ScenarioAssignment_1_5_4");
					put(grammarAccess.getFunctionAccess().getScenarioAssignment_1_7_2(), "rule__Function__ScenarioAssignment_1_7_2");
					put(grammarAccess.getFunctionAccess().getScenarioAssignment_1_8_2(), "rule__Function__ScenarioAssignment_1_8_2");
					put(grammarAccess.getFunctionAccess().getResourceAssignment_1_10_2(), "rule__Function__ResourceAssignment_1_10_2");
					put(grammarAccess.getFunctionAccess().getScenarioAssignment_1_10_4(), "rule__Function__ScenarioAssignment_1_10_4");
					put(grammarAccess.getHAlignAccess().getExpressionAssignment_0(), "rule__HAlign__ExpressionAssignment_0");
					put(grammarAccess.getHAlignAccess().getJustificationAssignment_1(), "rule__HAlign__JustificationAssignment_1");
					put(grammarAccess.getHideAccountAccess().getExpressionAssignment_1(), "rule__HideAccount__ExpressionAssignment_1");
					put(grammarAccess.getHideJournalEntryAccess().getExpressionAssignment_1(), "rule__HideJournalEntry__ExpressionAssignment_1");
					put(grammarAccess.getHideReportAccess().getExpressionAssignment_1(), "rule__HideReport__ExpressionAssignment_1");
					put(grammarAccess.getHideResourceAccess().getExpressionAssignment_1(), "rule__HideResource__ExpressionAssignment_1");
					put(grammarAccess.getHideTaskAccess().getExpressionAssignment_1(), "rule__HideTask__ExpressionAssignment_1");
					put(grammarAccess.getIncludeAccess().getImportURIAssignment_1(), "rule__Include__ImportURIAssignment_1");
					put(grammarAccess.getIncludePropertiesAccess().getImportURIAssignment_1(), "rule__IncludeProperties__ImportURIAssignment_1");
					put(grammarAccess.getIncludePropertiesAccess().getAttributesAssignment_2_1(), "rule__IncludeProperties__AttributesAssignment_2_1");
					put(grammarAccess.getInterval1Access().getStartAssignment_0(), "rule__Interval1__StartAssignment_0");
					put(grammarAccess.getInterval1Access().getEndAssignment_1_0_1(), "rule__Interval1__EndAssignment_1_0_1");
					put(grammarAccess.getInterval1Access().getDurationAssignment_1_1_1(), "rule__Interval1__DurationAssignment_1_1_1");
					put(grammarAccess.getInterval2Access().getStartAssignment_0(), "rule__Interval2__StartAssignment_0");
					put(grammarAccess.getInterval2Access().getEndAssignment_1_0_1(), "rule__Interval2__EndAssignment_1_0_1");
					put(grammarAccess.getInterval2Access().getDurationAssignment_1_1_1(), "rule__Interval2__DurationAssignment_1_1_1");
					put(grammarAccess.getInterval3Access().getStartAssignment_0(), "rule__Interval3__StartAssignment_0");
					put(grammarAccess.getInterval3Access().getEndAssignment_1_0_1(), "rule__Interval3__EndAssignment_1_0_1");
					put(grammarAccess.getInterval3Access().getDurationAssignment_1_1_1(), "rule__Interval3__DurationAssignment_1_1_1");
					put(grammarAccess.getInterval4Access().getStartAssignment_0(), "rule__Interval4__StartAssignment_0");
					put(grammarAccess.getInterval4Access().getEndAssignment_1_0_1(), "rule__Interval4__EndAssignment_1_0_1");
					put(grammarAccess.getInterval4Access().getDurationAssignment_1_1_1(), "rule__Interval4__DurationAssignment_1_1_1");
					put(grammarAccess.getJournalAttributesAccess().getAllAssignment_1_0(), "rule__JournalAttributes__AllAssignment_1_0");
					put(grammarAccess.getJournalAttributesAccess().getNoneAssignment_1_1(), "rule__JournalAttributes__NoneAssignment_1_1");
					put(grammarAccess.getJournalAttributesAccess().getFlagsAssignment_1_2_1_0(), "rule__JournalAttributes__FlagsAssignment_1_2_1_0");
					put(grammarAccess.getJournalAttributesAccess().getPropertyAssignment_1_2_1_1(), "rule__JournalAttributes__PropertyAssignment_1_2_1_1");
					put(grammarAccess.getJournalAttributesAccess().getDetailsAssignment_1_2_1_2(), "rule__JournalAttributes__DetailsAssignment_1_2_1_2");
					put(grammarAccess.getJournalAttributesAccess().getAuthorAssignment_1_2_1_3(), "rule__JournalAttributes__AuthorAssignment_1_2_1_3");
					put(grammarAccess.getJournalAttributesAccess().getHeadlineAssignment_1_2_1_4(), "rule__JournalAttributes__HeadlineAssignment_1_2_1_4");
					put(grammarAccess.getJournalAttributesAccess().getDateAssignment_1_2_1_5(), "rule__JournalAttributes__DateAssignment_1_2_1_5");
					put(grammarAccess.getJournalAttributesAccess().getTimesheetAssignment_1_2_1_6(), "rule__JournalAttributes__TimesheetAssignment_1_2_1_6");
					put(grammarAccess.getJournalAttributesAccess().getPropertyidAssignment_1_2_1_7(), "rule__JournalAttributes__PropertyidAssignment_1_2_1_7");
					put(grammarAccess.getJournalAttributesAccess().getSummaryAssignment_1_2_1_8(), "rule__JournalAttributes__SummaryAssignment_1_2_1_8");
					put(grammarAccess.getJournalEntryAccess().getDateAssignment_1(), "rule__JournalEntry__DateAssignment_1");
					put(grammarAccess.getJournalEntryAccess().getHeadlineAssignment_2(), "rule__JournalEntry__HeadlineAssignment_2");
					put(grammarAccess.getJournalEntryAccess().getAlertAssignment_3_1_0(), "rule__JournalEntry__AlertAssignment_3_1_0");
					put(grammarAccess.getJournalEntryAccess().getAuthorAssignment_3_1_1(), "rule__JournalEntry__AuthorAssignment_3_1_1");
					put(grammarAccess.getJournalEntryAccess().getDetailsAssignment_3_1_2(), "rule__JournalEntry__DetailsAssignment_3_1_2");
					put(grammarAccess.getJournalEntryAccess().getSummaryAssignment_3_1_3(), "rule__JournalEntry__SummaryAssignment_3_1_3");
					put(grammarAccess.getJournalModeAccess().getModeAssignment_1(), "rule__JournalMode__ModeAssignment_1");
					put(grammarAccess.getLengthAccess().getLengthAssignment_1(), "rule__Length__LengthAssignment_1");
					put(grammarAccess.getLimitsAccess().getAttributesAssignment_2_1(), "rule__Limits__AttributesAssignment_2_1");
					put(grammarAccess.getListTypeAccess().getTypeAssignment_1(), "rule__ListType__TypeAssignment_1");
					put(grammarAccess.getLoadUnitAccess().getUnitAssignment_1(), "rule__LoadUnit__UnitAssignment_1");
					put(grammarAccess.getLogicalOrExpressionAccess().getFeatureAssignment_1_0_0_1(), "rule__LogicalOrExpression__FeatureAssignment_1_0_0_1");
					put(grammarAccess.getLogicalOrExpressionAccess().getRightOperandAssignment_1_1(), "rule__LogicalOrExpression__RightOperandAssignment_1_1");
					put(grammarAccess.getLogicalAndExpressionAccess().getFeatureAssignment_1_0_0_1(), "rule__LogicalAndExpression__FeatureAssignment_1_0_0_1");
					put(grammarAccess.getLogicalAndExpressionAccess().getRightOperandAssignment_1_1(), "rule__LogicalAndExpression__RightOperandAssignment_1_1");
					put(grammarAccess.getLogicalEqualityExpressionAccess().getFeatureAssignment_1_0_0_1(), "rule__LogicalEqualityExpression__FeatureAssignment_1_0_0_1");
					put(grammarAccess.getLogicalEqualityExpressionAccess().getRightOperandAssignment_1_1(), "rule__LogicalEqualityExpression__RightOperandAssignment_1_1");
					put(grammarAccess.getLogicalRelationalExpressionAccess().getFeatureAssignment_1_0_0_1(), "rule__LogicalRelationalExpression__FeatureAssignment_1_0_0_1");
					put(grammarAccess.getLogicalRelationalExpressionAccess().getRightOperandAssignment_1_1(), "rule__LogicalRelationalExpression__RightOperandAssignment_1_1");
					put(grammarAccess.getLogicalFunctionExpressionAccess().getFunctionAssignment_1(), "rule__LogicalFunctionExpression__FunctionAssignment_1");
					put(grammarAccess.getLogicalAbsoluteIdExpressionAccess().getValueAssignment_1(), "rule__LogicalAbsoluteIdExpression__ValueAssignment_1");
					put(grammarAccess.getLogicalBooleanLiteralAccess().getIsTrueAssignment_1_1(), "rule__LogicalBooleanLiteral__IsTrueAssignment_1_1");
					put(grammarAccess.getLogicalNumeralLiteralAccess().getValueAssignment_1(), "rule__LogicalNumeralLiteral__ValueAssignment_1");
					put(grammarAccess.getLogicalStringLiteralAccess().getValueAssignment_1(), "rule__LogicalStringLiteral__ValueAssignment_1");
					put(grammarAccess.getLogicalISODateLiteralAccess().getValueAssignment_1(), "rule__LogicalISODateLiteral__ValueAssignment_1");
					put(grammarAccess.getMacroAccess().getValueAssignment_1(), "rule__Macro__ValueAssignment_1");
					put(grammarAccess.getManagersAccess().getResourcesAssignment_1(), "rule__Managers__ResourcesAssignment_1");
					put(grammarAccess.getManagersAccess().getResourcesAssignment_2_1(), "rule__Managers__ResourcesAssignment_2_1");
					put(grammarAccess.getMandatoryAccess().getMandatoryAssignment(), "rule__Mandatory__MandatoryAssignment");
					put(grammarAccess.getMaxEndAccess().getMaxEndAssignment_1(), "rule__MaxEnd__MaxEndAssignment_1");
					put(grammarAccess.getMaxStartAccess().getMaxStartAssignment_1(), "rule__MaxStart__MaxStartAssignment_1");
					put(grammarAccess.getMilestoneAccess().getMilestoneAssignment(), "rule__Milestone__MilestoneAssignment");
					put(grammarAccess.getMinEndAccess().getMinEndAssignment_1(), "rule__MinEnd__MinEndAssignment_1");
					put(grammarAccess.getMinStartAccess().getMinStartAssignment_1(), "rule__MinStart__MinStartAssignment_1");
					put(grammarAccess.getNoteAccess().getNoteAssignment_1(), "rule__Note__NoteAssignment_1");
					put(grammarAccess.getNowAccess().getNowAssignment_1(), "rule__Now__NowAssignment_1");
					put(grammarAccess.getPeriodAccess().getPeriodAssignment_1(), "rule__Period__PeriodAssignment_1");
					put(grammarAccess.getPersistentAccess().getPersistentAssignment(), "rule__Persistent__PersistentAssignment");
					put(grammarAccess.getPriorityAccess().getPriorityAssignment_1(), "rule__Priority__PriorityAssignment_1");
					put(grammarAccess.getProjectIdAccess().getProjectIdAssignment_1(), "rule__ProjectId__ProjectIdAssignment_1");
					put(grammarAccess.getProjectIdsAccess().getIdsAssignment_1(), "rule__ProjectIds__IdsAssignment_1");
					put(grammarAccess.getProjectIdsAccess().getIdsAssignment_2_1(), "rule__ProjectIds__IdsAssignment_2_1");
					put(grammarAccess.getPurgeReportAccess().getListAttributeAssignment_1(), "rule__PurgeReport__ListAttributeAssignment_1");
					put(grammarAccess.getPurgeResourceAccess().getListAttributeAssignment_1(), "rule__PurgeResource__ListAttributeAssignment_1");
					put(grammarAccess.getPurgeTaskAccess().getListAttributeAssignment_1(), "rule__PurgeTask__ListAttributeAssignment_1");
					put(grammarAccess.getRateAccess().getRateAssignment_1(), "rule__Rate__RateAssignment_1");
					put(grammarAccess.getRemainingAccess().getRemainingAssignment_1(), "rule__Remaining__RemainingAssignment_1");
					put(grammarAccess.getReportPrefixAccess().getReportAssignment_1(), "rule__ReportPrefix__ReportAssignment_1");
					put(grammarAccess.getResourceAttributesAccess().getAllAssignment_1_0(), "rule__ResourceAttributes__AllAssignment_1_0");
					put(grammarAccess.getResourceAttributesAccess().getNoneAssignment_1_1(), "rule__ResourceAttributes__NoneAssignment_1_1");
					put(grammarAccess.getResourceAttributesAccess().getVacationAssignment_1_2_1_0(), "rule__ResourceAttributes__VacationAssignment_1_2_1_0");
					put(grammarAccess.getResourceAttributesAccess().getBookingAssignment_1_2_1_1(), "rule__ResourceAttributes__BookingAssignment_1_2_1_1");
					put(grammarAccess.getResourceAttributesAccess().getWorkingHoursAssignment_1_2_1_2(), "rule__ResourceAttributes__WorkingHoursAssignment_1_2_1_2");
					put(grammarAccess.getResourcePrefixAccess().getResourceAssignment_1(), "rule__ResourcePrefix__ResourceAssignment_1");
					put(grammarAccess.getResourceRootAccess().getResourceAssignment_1(), "rule__ResourceRoot__ResourceAssignment_1");
					put(grammarAccess.getResponsibleAccess().getResourcesAssignment_1(), "rule__Responsible__ResourcesAssignment_1");
					put(grammarAccess.getResponsibleAccess().getResourcesAssignment_2_1(), "rule__Responsible__ResourcesAssignment_2_1");
					put(grammarAccess.getRGBAccess().getValueAssignment(), "rule__RGB__ValueAssignment");
					put(grammarAccess.getRollupAccountAccess().getExpressionAssignment_1(), "rule__RollupAccount__ExpressionAssignment_1");
					put(grammarAccess.getRollupResourceAccess().getExpressionAssignment_1(), "rule__RollupResource__ExpressionAssignment_1");
					put(grammarAccess.getRollupTaskAccess().getExpressionAssignment_1(), "rule__RollupTask__ExpressionAssignment_1");
					put(grammarAccess.getScaleAccess().getScaleAssignment_1(), "rule__Scale__ScaleAssignment_1");
					put(grammarAccess.getScenarioAccess().getIdAssignment_1(), "rule__Scenario__IdAssignment_1");
					put(grammarAccess.getScenarioAccess().getNameAssignment_2(), "rule__Scenario__NameAssignment_2");
					put(grammarAccess.getScenarioAccess().getActiveAssignment_3_1_0_1(), "rule__Scenario__ActiveAssignment_3_1_0_1");
					put(grammarAccess.getScenarioAccess().getScenarioAssignment_3_1_1(), "rule__Scenario__ScenarioAssignment_3_1_1");
					put(grammarAccess.getScenarioIcalAccess().getScenarioAssignment_1(), "rule__ScenarioIcal__ScenarioAssignment_1");
					put(grammarAccess.getScenariosAccess().getScenariosAssignment_1(), "rule__Scenarios__ScenariosAssignment_1");
					put(grammarAccess.getScenariosAccess().getScenariosAssignment_2_1(), "rule__Scenarios__ScenariosAssignment_2_1");
					put(grammarAccess.getScheduledAccess().getScheduledAssignment(), "rule__Scheduled__ScheduledAssignment");
					put(grammarAccess.getSchedulingAccess().getSchedulingAssignment_1(), "rule__Scheduling__SchedulingAssignment_1");
					put(grammarAccess.getSelectAccess().getArgumentAssignment_1(), "rule__Select__ArgumentAssignment_1");
					put(grammarAccess.getSelfContainedAccess().getSelfcontainedAssignment_1(), "rule__SelfContained__SelfcontainedAssignment_1");
					put(grammarAccess.getShiftAccess().getIdAssignment_1(), "rule__Shift__IdAssignment_1");
					put(grammarAccess.getShiftAccess().getNameAssignment_2(), "rule__Shift__NameAssignment_2");
					put(grammarAccess.getShiftAccess().getReplaceAssignment_3_1_0(), "rule__Shift__ReplaceAssignment_3_1_0");
					put(grammarAccess.getShiftAccess().getTimezoneAssignment_3_1_1_1(), "rule__Shift__TimezoneAssignment_3_1_1_1");
					put(grammarAccess.getShiftAccess().getVacationAssignment_3_1_2_1(), "rule__Shift__VacationAssignment_3_1_2_1");
					put(grammarAccess.getShiftAccess().getShiftAssignment_3_1_3_1(), "rule__Shift__ShiftAssignment_3_1_3_1");
					put(grammarAccess.getShiftAccess().getWorkingHoursAssignment_3_1_4_1(), "rule__Shift__WorkingHoursAssignment_3_1_4_1");
					put(grammarAccess.getShiftTimesheetAccess().getShiftAssignment_1(), "rule__ShiftTimesheet__ShiftAssignment_1");
					put(grammarAccess.getShiftsAccess().getLimitsAssignment_1(), "rule__Shifts__LimitsAssignment_1");
					put(grammarAccess.getShiftsAccess().getLimitsAssignment_2_1(), "rule__Shifts__LimitsAssignment_2_1");
					put(grammarAccess.getShiftsLimitAccess().getShiftAssignment_0(), "rule__ShiftsLimit__ShiftAssignment_0");
					put(grammarAccess.getShiftsLimitAccess().getLimitAssignment_1(), "rule__ShiftsLimit__LimitAssignment_1");
					put(grammarAccess.getShiftsAllocateAccess().getShiftAssignment_1(), "rule__ShiftsAllocate__ShiftAssignment_1");
					put(grammarAccess.getShiftsAllocateAccess().getIntervalsAssignment_2_0(), "rule__ShiftsAllocate__IntervalsAssignment_2_0");
					put(grammarAccess.getShiftsAllocateAccess().getIntervalsAssignment_2_1_1(), "rule__ShiftsAllocate__IntervalsAssignment_2_1_1");
					put(grammarAccess.getShortTimeFormatAccess().getShortTimeFormatAssignment_1(), "rule__ShortTimeFormat__ShortTimeFormatAssignment_1");
					put(grammarAccess.getSortAccess().getTreeAssignment_1_0(), "rule__Sort__TreeAssignment_1_0");
					put(grammarAccess.getSortAccess().getCriteriaAssignment_1_1(), "rule__Sort__CriteriaAssignment_1_1");
					put(grammarAccess.getSortAccess().getCriteriaAssignment_2_1(), "rule__Sort__CriteriaAssignment_2_1");
					put(grammarAccess.getStartAccess().getStartAssignment_1(), "rule__Start__StartAssignment_1");
					put(grammarAccess.getStatusStatusSheetAccess().getLevelAssignment_1(), "rule__StatusStatusSheet__LevelAssignment_1");
					put(grammarAccess.getStatusStatusSheetAccess().getTextAssignment_2(), "rule__StatusStatusSheet__TextAssignment_2");
					put(grammarAccess.getStatusStatusSheetAccess().getAttributesAssignment_3_1(), "rule__StatusStatusSheet__AttributesAssignment_3_1");
					put(grammarAccess.getStatusTimesheetAccess().getLevelAssignment_1(), "rule__StatusTimesheet__LevelAssignment_1");
					put(grammarAccess.getStatusTimesheetAccess().getTextAssignment_2(), "rule__StatusTimesheet__TextAssignment_2");
					put(grammarAccess.getStatusTimesheetAccess().getAttributesAssignment_3_1(), "rule__StatusTimesheet__AttributesAssignment_3_1");
					put(grammarAccess.getStatusSheetAccess().getResourceAssignment_1(), "rule__StatusSheet__ResourceAssignment_1");
					put(grammarAccess.getStatusSheetAccess().getIntervalAssignment_2(), "rule__StatusSheet__IntervalAssignment_2");
					put(grammarAccess.getStatusSheetAccess().getAttributesAssignment_3_1(), "rule__StatusSheet__AttributesAssignment_3_1");
					put(grammarAccess.getStatusSheetReportAccess().getFilenameAssignment_1(), "rule__StatusSheetReport__FilenameAssignment_1");
					put(grammarAccess.getStatusSheetReportAccess().getAttributesAssignment_2_1(), "rule__StatusSheetReport__AttributesAssignment_2_1");
					put(grammarAccess.getSupplementAccountAccess().getAccountAssignment_2(), "rule__SupplementAccount__AccountAssignment_2");
					put(grammarAccess.getSupplementAccountAccess().getAttributesAssignment_3_1(), "rule__SupplementAccount__AttributesAssignment_3_1");
					put(grammarAccess.getSupplementReportAccess().getReportAssignment_2(), "rule__SupplementReport__ReportAssignment_2");
					put(grammarAccess.getSupplementReportAccess().getAttributesAssignment_3_1(), "rule__SupplementReport__AttributesAssignment_3_1");
					put(grammarAccess.getSupplementResourceAccess().getResourceAssignment_2(), "rule__SupplementResource__ResourceAssignment_2");
					put(grammarAccess.getSupplementResourceAccess().getAttributesAssignment_3_1(), "rule__SupplementResource__AttributesAssignment_3_1");
					put(grammarAccess.getSupplementTaskAccess().getTaskAssignment_2(), "rule__SupplementTask__TaskAssignment_2");
					put(grammarAccess.getSupplementTaskAccess().getAttributesAssignment_3_1(), "rule__SupplementTask__AttributesAssignment_3_1");
					put(grammarAccess.getTagFileAccess().getIdAssignment_1(), "rule__TagFile__IdAssignment_1");
					put(grammarAccess.getTagFileAccess().getFilenameAssignment_2(), "rule__TagFile__FilenameAssignment_2");
					put(grammarAccess.getTagFileAccess().getHideResourceAssignment_3_0_1(), "rule__TagFile__HideResourceAssignment_3_0_1");
					put(grammarAccess.getTagFileAccess().getHideTaskAssignment_3_1(), "rule__TagFile__HideTaskAssignment_3_1");
					put(grammarAccess.getTagFileAccess().getRollupResourceAssignment_3_2(), "rule__TagFile__RollupResourceAssignment_3_2");
					put(grammarAccess.getTagFileAccess().getRollupTaskAssignment_3_3_0(), "rule__TagFile__RollupTaskAssignment_3_3_0");
					put(grammarAccess.getTaskStatusSheetAccess().getTaskAssignment_1(), "rule__TaskStatusSheet__TaskAssignment_1");
					put(grammarAccess.getTaskStatusSheetAccess().getAttributesAssignment_2_1(), "rule__TaskStatusSheet__AttributesAssignment_2_1");
					put(grammarAccess.getTaskTimesheetAccess().getTaskAssignment_1(), "rule__TaskTimesheet__TaskAssignment_1");
					put(grammarAccess.getTaskTimesheetAccess().getAttributesAssignment_2_1(), "rule__TaskTimesheet__AttributesAssignment_2_1");
					put(grammarAccess.getTaskAttributesAccess().getAllAssignment_1_0(), "rule__TaskAttributes__AllAssignment_1_0");
					put(grammarAccess.getTaskAttributesAccess().getNoneAssignment_1_1(), "rule__TaskAttributes__NoneAssignment_1_1");
					put(grammarAccess.getTaskAttributesAccess().getResponsibleAssignment_1_2_1_0(), "rule__TaskAttributes__ResponsibleAssignment_1_2_1_0");
					put(grammarAccess.getTaskAttributesAccess().getFlagsAssignment_1_2_1_1(), "rule__TaskAttributes__FlagsAssignment_1_2_1_1");
					put(grammarAccess.getTaskAttributesAccess().getMaxstartAssignment_1_2_1_2(), "rule__TaskAttributes__MaxstartAssignment_1_2_1_2");
					put(grammarAccess.getTaskAttributesAccess().getMaxendAssignment_1_2_1_3(), "rule__TaskAttributes__MaxendAssignment_1_2_1_3");
					put(grammarAccess.getTaskAttributesAccess().getPriorityAssignment_1_2_1_4(), "rule__TaskAttributes__PriorityAssignment_1_2_1_4");
					put(grammarAccess.getTaskAttributesAccess().getBookingAssignment_1_2_1_5(), "rule__TaskAttributes__BookingAssignment_1_2_1_5");
					put(grammarAccess.getTaskAttributesAccess().getNoteAssignment_1_2_1_6(), "rule__TaskAttributes__NoteAssignment_1_2_1_6");
					put(grammarAccess.getTaskAttributesAccess().getMinstartAssignment_1_2_1_7(), "rule__TaskAttributes__MinstartAssignment_1_2_1_7");
					put(grammarAccess.getTaskAttributesAccess().getMinendAssignment_1_2_1_8(), "rule__TaskAttributes__MinendAssignment_1_2_1_8");
					put(grammarAccess.getTaskAttributesAccess().getCompleteAssignment_1_2_1_9(), "rule__TaskAttributes__CompleteAssignment_1_2_1_9");
					put(grammarAccess.getTaskAttributesAccess().getDependsAssignment_1_2_1_10(), "rule__TaskAttributes__DependsAssignment_1_2_1_10");
					put(grammarAccess.getTaskPrefixAccess().getTaskAssignment_1(), "rule__TaskPrefix__TaskAssignment_1");
					put(grammarAccess.getTaskRootAccess().getTaskAssignment_1(), "rule__TaskRoot__TaskAssignment_1");
					put(grammarAccess.getTimeFormatAccess().getTimeformatAssignment_1(), "rule__TimeFormat__TimeformatAssignment_1");
					put(grammarAccess.getTimeoffAccess().getIdAssignment_1(), "rule__Timeoff__IdAssignment_1");
					put(grammarAccess.getTimeoffAccess().getNameAssignment_2(), "rule__Timeoff__NameAssignment_2");
					put(grammarAccess.getTimesheetAccess().getResourceAssignment_1(), "rule__Timesheet__ResourceAssignment_1");
					put(grammarAccess.getTimesheetAccess().getIntervalAssignment_2(), "rule__Timesheet__IntervalAssignment_2");
					put(grammarAccess.getTimesheetAccess().getAttributesAssignment_3_1(), "rule__Timesheet__AttributesAssignment_3_1");
					put(grammarAccess.getTimesheetReportAccess().getFilenameAssignment_1(), "rule__TimesheetReport__FilenameAssignment_1");
					put(grammarAccess.getTimesheetReportAccess().getAttributesAssignment_2_1(), "rule__TimesheetReport__AttributesAssignment_2_1");
					put(grammarAccess.getTimezoneAccess().getTimezoneAssignment_1(), "rule__Timezone__TimezoneAssignment_1");
					put(grammarAccess.getTimingResolutionAccess().getTimingResolutionAssignment_1(), "rule__TimingResolution__TimingResolutionAssignment_1");
					put(grammarAccess.getTitleAccess().getTitleAssignment_1(), "rule__Title__TitleAssignment_1");
					put(grammarAccess.getToolTipAccess().getExpressionAssignment_1(), "rule__ToolTip__ExpressionAssignment_1");
					put(grammarAccess.getToolTipAccess().getTipAssignment_2(), "rule__ToolTip__TipAssignment_2");
					put(grammarAccess.getTrackingScenarioAccess().getScenarioAssignment_1(), "rule__TrackingScenario__ScenarioAssignment_1");
					put(grammarAccess.getTreeLevelAccess().getLevelAssignment_1(), "rule__TreeLevel__LevelAssignment_1");
					put(grammarAccess.getVacationAccess().getNameAssignment_1(), "rule__Vacation__NameAssignment_1");
					put(grammarAccess.getVacationAccess().getIntervalsAssignment_2(), "rule__Vacation__IntervalsAssignment_2");
					put(grammarAccess.getVacationAccess().getIntervalsAssignment_3_1(), "rule__Vacation__IntervalsAssignment_3_1");
					put(grammarAccess.getVacationShiftAccess().getNameAssignment_2(), "rule__VacationShift__NameAssignment_2");
					put(grammarAccess.getVacationShiftAccess().getIntervalsAssignment_3_0(), "rule__VacationShift__IntervalsAssignment_3_0");
					put(grammarAccess.getVacationShiftAccess().getIntervalsAssignment_3_1_1(), "rule__VacationShift__IntervalsAssignment_3_1_1");
					put(grammarAccess.getVacationResourceAccess().getNameAssignment_1(), "rule__VacationResource__NameAssignment_1");
					put(grammarAccess.getVacationResourceAccess().getIntervalsAssignment_2(), "rule__VacationResource__IntervalsAssignment_2");
					put(grammarAccess.getVacationResourceAccess().getIntervalsAssignment_3_1(), "rule__VacationResource__IntervalsAssignment_3_1");
					put(grammarAccess.getWarnAccess().getExpressionAssignment_1(), "rule__Warn__ExpressionAssignment_1");
					put(grammarAccess.getWeekStartsAccess().getSundayAssignment_0(), "rule__WeekStarts__SundayAssignment_0");
					put(grammarAccess.getWeekStartsAccess().getMondayAssignment_1(), "rule__WeekStarts__MondayAssignment_1");
					put(grammarAccess.getWidthAccess().getWidthAssignment_1(), "rule__Width__WidthAssignment_1");
					put(grammarAccess.getWorkAccess().getValueAssignment_1(), "rule__Work__ValueAssignment_1");
					put(grammarAccess.getWorkAccess().getUnitAssignment_2(), "rule__Work__UnitAssignment_2");
					put(grammarAccess.getWorkingHoursAccess().getWeekdaysAssignment_1_0(), "rule__WorkingHours__WeekdaysAssignment_1_0");
					put(grammarAccess.getWorkingHoursAccess().getWeekdaysAssignment_1_1_1(), "rule__WorkingHours__WeekdaysAssignment_1_1_1");
					put(grammarAccess.getWorkingHoursAccess().getOffAssignment_2_0(), "rule__WorkingHours__OffAssignment_2_0");
					put(grammarAccess.getWorkingHoursAccess().getHoursAssignment_2_1_0(), "rule__WorkingHours__HoursAssignment_2_1_0");
					put(grammarAccess.getWorkingHoursAccess().getHoursAssignment_2_1_1_1(), "rule__WorkingHours__HoursAssignment_2_1_1_1");
					put(grammarAccess.getYearlyWorkingDaysAccess().getYearlyWorkingDaysAssignment_1(), "rule__YearlyWorkingDays__YearlyWorkingDaysAssignment_1");
					put(grammarAccess.getAccountShareAccess().getAccountAssignment_0(), "rule__AccountShare__AccountAssignment_0");
					put(grammarAccess.getAccountShareAccess().getShareAssignment_1(), "rule__AccountShare__ShareAssignment_1");
					put(grammarAccess.getColumnAccess().getIdAssignment_0(), "rule__Column__IdAssignment_0");
					put(grammarAccess.getColumnAccess().getAttributeAssignment_1(), "rule__Column__AttributeAssignment_1");
					put(grammarAccess.getCriterionAccess().getColumnIdAssignment_0(), "rule__Criterion__ColumnIdAssignment_0");
					put(grammarAccess.getCriterionAccess().getDirectionAssignment_2(), "rule__Criterion__DirectionAssignment_2");
					put(grammarAccess.getDurationQuantityAccess().getValueAssignment_0(), "rule__DurationQuantity__ValueAssignment_0");
					put(grammarAccess.getDurationQuantityAccess().getUnitAssignment_1(), "rule__DurationQuantity__UnitAssignment_1");
					put(grammarAccess.getLimitAccess().getDurationAssignment_0(), "rule__Limit__DurationAssignment_0");
					put(grammarAccess.getLimitAccess().getAttributesAssignment_1_1(), "rule__Limit__AttributesAssignment_1_1");
					put(grammarAccess.getLimitAttributeAccess().getEndAssignment_0_1(), "rule__LimitAttribute__EndAssignment_0_1");
					put(grammarAccess.getLimitAttributeAccess().getPeriodAssignment_1_1(), "rule__LimitAttribute__PeriodAssignment_1_1");
					put(grammarAccess.getLimitAttributeAccess().getResourcesAssignment_2_1(), "rule__LimitAttribute__ResourcesAssignment_2_1");
					put(grammarAccess.getLimitAttributeAccess().getResourcesAssignment_2_2_1(), "rule__LimitAttribute__ResourcesAssignment_2_2_1");
					put(grammarAccess.getLimitAttributeAccess().getStartAssignment_3_1(), "rule__LimitAttribute__StartAssignment_3_1");
					put(grammarAccess.getRealFormatAccess().getNegativePrefixAssignment_0(), "rule__RealFormat__NegativePrefixAssignment_0");
					put(grammarAccess.getRealFormatAccess().getNegativeSuffixAssignment_1(), "rule__RealFormat__NegativeSuffixAssignment_1");
					put(grammarAccess.getRealFormatAccess().getThousandsSeparatorAssignment_2(), "rule__RealFormat__ThousandsSeparatorAssignment_2");
					put(grammarAccess.getRealFormatAccess().getFractionSeparatorAssignment_3(), "rule__RealFormat__FractionSeparatorAssignment_3");
					put(grammarAccess.getRealFormatAccess().getFractionDigitsAssignment_4(), "rule__RealFormat__FractionDigitsAssignment_4");
					put(grammarAccess.getTaskDependencyAccess().getTaskAssignment_0(), "rule__TaskDependency__TaskAssignment_0");
					put(grammarAccess.getTaskDependencyAccess().getGapDurationAssignment_1_1_0(), "rule__TaskDependency__GapDurationAssignment_1_1_0");
					put(grammarAccess.getTaskDependencyAccess().getGapLengthAssignment_1_1_1(), "rule__TaskDependency__GapLengthAssignment_1_1_1");
					put(grammarAccess.getTaskDependencyAccess().getPolicyAssignment_1_1_2(), "rule__TaskDependency__PolicyAssignment_1_1_2");
					put(grammarAccess.getRichTextAccess().getTextAssignment(), "rule__RichText__TextAssignment");
					put(grammarAccess.getWorkHoursAccess().getStartAssignment_0(), "rule__WorkHours__StartAssignment_0");
					put(grammarAccess.getWorkHoursAccess().getStopAssignment_2(), "rule__WorkHours__StopAssignment_2");
					put(grammarAccess.getWeekdaysAccess().getFirstAssignment_0(), "rule__Weekdays__FirstAssignment_0");
					put(grammarAccess.getWeekdaysAccess().getLastAssignment_1_1(), "rule__Weekdays__LastAssignment_1_1");
					put(grammarAccess.getXAssignmentAccess().getFeatureAssignment_0_1(), "rule__XAssignment__FeatureAssignment_0_1");
					put(grammarAccess.getXAssignmentAccess().getValueAssignment_0_3(), "rule__XAssignment__ValueAssignment_0_3");
					put(grammarAccess.getXAssignmentAccess().getFeatureAssignment_1_1_0_0_1(), "rule__XAssignment__FeatureAssignment_1_1_0_0_1");
					put(grammarAccess.getXAssignmentAccess().getRightOperandAssignment_1_1_1(), "rule__XAssignment__RightOperandAssignment_1_1_1");
					put(grammarAccess.getXOrExpressionAccess().getFeatureAssignment_1_0_0_1(), "rule__XOrExpression__FeatureAssignment_1_0_0_1");
					put(grammarAccess.getXOrExpressionAccess().getRightOperandAssignment_1_1(), "rule__XOrExpression__RightOperandAssignment_1_1");
					put(grammarAccess.getXAndExpressionAccess().getFeatureAssignment_1_0_0_1(), "rule__XAndExpression__FeatureAssignment_1_0_0_1");
					put(grammarAccess.getXAndExpressionAccess().getRightOperandAssignment_1_1(), "rule__XAndExpression__RightOperandAssignment_1_1");
					put(grammarAccess.getXEqualityExpressionAccess().getFeatureAssignment_1_0_0_1(), "rule__XEqualityExpression__FeatureAssignment_1_0_0_1");
					put(grammarAccess.getXEqualityExpressionAccess().getRightOperandAssignment_1_1(), "rule__XEqualityExpression__RightOperandAssignment_1_1");
					put(grammarAccess.getXRelationalExpressionAccess().getTypeAssignment_1_0_1(), "rule__XRelationalExpression__TypeAssignment_1_0_1");
					put(grammarAccess.getXRelationalExpressionAccess().getFeatureAssignment_1_1_0_0_1(), "rule__XRelationalExpression__FeatureAssignment_1_1_0_0_1");
					put(grammarAccess.getXRelationalExpressionAccess().getRightOperandAssignment_1_1_1(), "rule__XRelationalExpression__RightOperandAssignment_1_1_1");
					put(grammarAccess.getXOtherOperatorExpressionAccess().getFeatureAssignment_1_0_0_1(), "rule__XOtherOperatorExpression__FeatureAssignment_1_0_0_1");
					put(grammarAccess.getXOtherOperatorExpressionAccess().getRightOperandAssignment_1_1(), "rule__XOtherOperatorExpression__RightOperandAssignment_1_1");
					put(grammarAccess.getXAdditiveExpressionAccess().getFeatureAssignment_1_0_0_1(), "rule__XAdditiveExpression__FeatureAssignment_1_0_0_1");
					put(grammarAccess.getXAdditiveExpressionAccess().getRightOperandAssignment_1_1(), "rule__XAdditiveExpression__RightOperandAssignment_1_1");
					put(grammarAccess.getXMultiplicativeExpressionAccess().getFeatureAssignment_1_0_0_1(), "rule__XMultiplicativeExpression__FeatureAssignment_1_0_0_1");
					put(grammarAccess.getXMultiplicativeExpressionAccess().getRightOperandAssignment_1_1(), "rule__XMultiplicativeExpression__RightOperandAssignment_1_1");
					put(grammarAccess.getXUnaryOperationAccess().getFeatureAssignment_0_1(), "rule__XUnaryOperation__FeatureAssignment_0_1");
					put(grammarAccess.getXUnaryOperationAccess().getOperandAssignment_0_2(), "rule__XUnaryOperation__OperandAssignment_0_2");
					put(grammarAccess.getXCastedExpressionAccess().getTypeAssignment_1_1(), "rule__XCastedExpression__TypeAssignment_1_1");
					put(grammarAccess.getXMemberFeatureCallAccess().getFeatureAssignment_1_0_0_0_2(), "rule__XMemberFeatureCall__FeatureAssignment_1_0_0_0_2");
					put(grammarAccess.getXMemberFeatureCallAccess().getValueAssignment_1_0_1(), "rule__XMemberFeatureCall__ValueAssignment_1_0_1");
					put(grammarAccess.getXMemberFeatureCallAccess().getNullSafeAssignment_1_1_0_0_1_1(), "rule__XMemberFeatureCall__NullSafeAssignment_1_1_0_0_1_1");
					put(grammarAccess.getXMemberFeatureCallAccess().getSpreadingAssignment_1_1_0_0_1_2(), "rule__XMemberFeatureCall__SpreadingAssignment_1_1_0_0_1_2");
					put(grammarAccess.getXMemberFeatureCallAccess().getTypeArgumentsAssignment_1_1_1_1(), "rule__XMemberFeatureCall__TypeArgumentsAssignment_1_1_1_1");
					put(grammarAccess.getXMemberFeatureCallAccess().getTypeArgumentsAssignment_1_1_1_2_1(), "rule__XMemberFeatureCall__TypeArgumentsAssignment_1_1_1_2_1");
					put(grammarAccess.getXMemberFeatureCallAccess().getFeatureAssignment_1_1_2(), "rule__XMemberFeatureCall__FeatureAssignment_1_1_2");
					put(grammarAccess.getXMemberFeatureCallAccess().getExplicitOperationCallAssignment_1_1_3_0(), "rule__XMemberFeatureCall__ExplicitOperationCallAssignment_1_1_3_0");
					put(grammarAccess.getXMemberFeatureCallAccess().getMemberCallArgumentsAssignment_1_1_3_1_0(), "rule__XMemberFeatureCall__MemberCallArgumentsAssignment_1_1_3_1_0");
					put(grammarAccess.getXMemberFeatureCallAccess().getMemberCallArgumentsAssignment_1_1_3_1_1_0(), "rule__XMemberFeatureCall__MemberCallArgumentsAssignment_1_1_3_1_1_0");
					put(grammarAccess.getXMemberFeatureCallAccess().getMemberCallArgumentsAssignment_1_1_3_1_1_1_1(), "rule__XMemberFeatureCall__MemberCallArgumentsAssignment_1_1_3_1_1_1_1");
					put(grammarAccess.getXMemberFeatureCallAccess().getMemberCallArgumentsAssignment_1_1_4(), "rule__XMemberFeatureCall__MemberCallArgumentsAssignment_1_1_4");
					put(grammarAccess.getXClosureAccess().getDeclaredFormalParametersAssignment_2_0_0_0(), "rule__XClosure__DeclaredFormalParametersAssignment_2_0_0_0");
					put(grammarAccess.getXClosureAccess().getDeclaredFormalParametersAssignment_2_0_0_1_1(), "rule__XClosure__DeclaredFormalParametersAssignment_2_0_0_1_1");
					put(grammarAccess.getXClosureAccess().getExplicitSyntaxAssignment_2_0_1(), "rule__XClosure__ExplicitSyntaxAssignment_2_0_1");
					put(grammarAccess.getXClosureAccess().getExpressionAssignment_3(), "rule__XClosure__ExpressionAssignment_3");
					put(grammarAccess.getXExpressionInClosureAccess().getExpressionsAssignment_1_0(), "rule__XExpressionInClosure__ExpressionsAssignment_1_0");
					put(grammarAccess.getXShortClosureAccess().getDeclaredFormalParametersAssignment_0_0_1_0(), "rule__XShortClosure__DeclaredFormalParametersAssignment_0_0_1_0");
					put(grammarAccess.getXShortClosureAccess().getDeclaredFormalParametersAssignment_0_0_1_1_1(), "rule__XShortClosure__DeclaredFormalParametersAssignment_0_0_1_1_1");
					put(grammarAccess.getXShortClosureAccess().getExplicitSyntaxAssignment_0_0_2(), "rule__XShortClosure__ExplicitSyntaxAssignment_0_0_2");
					put(grammarAccess.getXShortClosureAccess().getExpressionAssignment_1(), "rule__XShortClosure__ExpressionAssignment_1");
					put(grammarAccess.getXIfExpressionAccess().getIfAssignment_3(), "rule__XIfExpression__IfAssignment_3");
					put(grammarAccess.getXIfExpressionAccess().getThenAssignment_5(), "rule__XIfExpression__ThenAssignment_5");
					put(grammarAccess.getXIfExpressionAccess().getElseAssignment_6_1(), "rule__XIfExpression__ElseAssignment_6_1");
					put(grammarAccess.getXSwitchExpressionAccess().getLocalVarNameAssignment_2_0_0(), "rule__XSwitchExpression__LocalVarNameAssignment_2_0_0");
					put(grammarAccess.getXSwitchExpressionAccess().getSwitchAssignment_3(), "rule__XSwitchExpression__SwitchAssignment_3");
					put(grammarAccess.getXSwitchExpressionAccess().getCasesAssignment_5(), "rule__XSwitchExpression__CasesAssignment_5");
					put(grammarAccess.getXSwitchExpressionAccess().getDefaultAssignment_6_2(), "rule__XSwitchExpression__DefaultAssignment_6_2");
					put(grammarAccess.getXCasePartAccess().getTypeGuardAssignment_0(), "rule__XCasePart__TypeGuardAssignment_0");
					put(grammarAccess.getXCasePartAccess().getCaseAssignment_1_1(), "rule__XCasePart__CaseAssignment_1_1");
					put(grammarAccess.getXCasePartAccess().getThenAssignment_3(), "rule__XCasePart__ThenAssignment_3");
					put(grammarAccess.getXForLoopExpressionAccess().getDeclaredParamAssignment_3(), "rule__XForLoopExpression__DeclaredParamAssignment_3");
					put(grammarAccess.getXForLoopExpressionAccess().getForExpressionAssignment_5(), "rule__XForLoopExpression__ForExpressionAssignment_5");
					put(grammarAccess.getXForLoopExpressionAccess().getEachExpressionAssignment_7(), "rule__XForLoopExpression__EachExpressionAssignment_7");
					put(grammarAccess.getXWhileExpressionAccess().getPredicateAssignment_3(), "rule__XWhileExpression__PredicateAssignment_3");
					put(grammarAccess.getXWhileExpressionAccess().getBodyAssignment_5(), "rule__XWhileExpression__BodyAssignment_5");
					put(grammarAccess.getXDoWhileExpressionAccess().getBodyAssignment_2(), "rule__XDoWhileExpression__BodyAssignment_2");
					put(grammarAccess.getXDoWhileExpressionAccess().getPredicateAssignment_5(), "rule__XDoWhileExpression__PredicateAssignment_5");
					put(grammarAccess.getXBlockExpressionAccess().getExpressionsAssignment_2_0(), "rule__XBlockExpression__ExpressionsAssignment_2_0");
					put(grammarAccess.getXVariableDeclarationAccess().getWriteableAssignment_1_0(), "rule__XVariableDeclaration__WriteableAssignment_1_0");
					put(grammarAccess.getXVariableDeclarationAccess().getTypeAssignment_2_0_0_0(), "rule__XVariableDeclaration__TypeAssignment_2_0_0_0");
					put(grammarAccess.getXVariableDeclarationAccess().getNameAssignment_2_0_0_1(), "rule__XVariableDeclaration__NameAssignment_2_0_0_1");
					put(grammarAccess.getXVariableDeclarationAccess().getNameAssignment_2_1(), "rule__XVariableDeclaration__NameAssignment_2_1");
					put(grammarAccess.getXVariableDeclarationAccess().getRightAssignment_3_1(), "rule__XVariableDeclaration__RightAssignment_3_1");
					put(grammarAccess.getJvmFormalParameterAccess().getParameterTypeAssignment_0(), "rule__JvmFormalParameter__ParameterTypeAssignment_0");
					put(grammarAccess.getJvmFormalParameterAccess().getNameAssignment_1(), "rule__JvmFormalParameter__NameAssignment_1");
					put(grammarAccess.getFullJvmFormalParameterAccess().getParameterTypeAssignment_0(), "rule__FullJvmFormalParameter__ParameterTypeAssignment_0");
					put(grammarAccess.getFullJvmFormalParameterAccess().getNameAssignment_1(), "rule__FullJvmFormalParameter__NameAssignment_1");
					put(grammarAccess.getXFeatureCallAccess().getDeclaringTypeAssignment_1(), "rule__XFeatureCall__DeclaringTypeAssignment_1");
					put(grammarAccess.getXFeatureCallAccess().getTypeArgumentsAssignment_2_1(), "rule__XFeatureCall__TypeArgumentsAssignment_2_1");
					put(grammarAccess.getXFeatureCallAccess().getTypeArgumentsAssignment_2_2_1(), "rule__XFeatureCall__TypeArgumentsAssignment_2_2_1");
					put(grammarAccess.getXFeatureCallAccess().getFeatureAssignment_3(), "rule__XFeatureCall__FeatureAssignment_3");
					put(grammarAccess.getXFeatureCallAccess().getExplicitOperationCallAssignment_4_0(), "rule__XFeatureCall__ExplicitOperationCallAssignment_4_0");
					put(grammarAccess.getXFeatureCallAccess().getFeatureCallArgumentsAssignment_4_1_0(), "rule__XFeatureCall__FeatureCallArgumentsAssignment_4_1_0");
					put(grammarAccess.getXFeatureCallAccess().getFeatureCallArgumentsAssignment_4_1_1_0(), "rule__XFeatureCall__FeatureCallArgumentsAssignment_4_1_1_0");
					put(grammarAccess.getXFeatureCallAccess().getFeatureCallArgumentsAssignment_4_1_1_1_1(), "rule__XFeatureCall__FeatureCallArgumentsAssignment_4_1_1_1_1");
					put(grammarAccess.getXFeatureCallAccess().getFeatureCallArgumentsAssignment_5(), "rule__XFeatureCall__FeatureCallArgumentsAssignment_5");
					put(grammarAccess.getXConstructorCallAccess().getConstructorAssignment_2(), "rule__XConstructorCall__ConstructorAssignment_2");
					put(grammarAccess.getXConstructorCallAccess().getTypeArgumentsAssignment_3_1(), "rule__XConstructorCall__TypeArgumentsAssignment_3_1");
					put(grammarAccess.getXConstructorCallAccess().getTypeArgumentsAssignment_3_2_1(), "rule__XConstructorCall__TypeArgumentsAssignment_3_2_1");
					put(grammarAccess.getXConstructorCallAccess().getArgumentsAssignment_5_0(), "rule__XConstructorCall__ArgumentsAssignment_5_0");
					put(grammarAccess.getXConstructorCallAccess().getArgumentsAssignment_5_1_0(), "rule__XConstructorCall__ArgumentsAssignment_5_1_0");
					put(grammarAccess.getXConstructorCallAccess().getArgumentsAssignment_5_1_1_1(), "rule__XConstructorCall__ArgumentsAssignment_5_1_1_1");
					put(grammarAccess.getXConstructorCallAccess().getArgumentsAssignment_7(), "rule__XConstructorCall__ArgumentsAssignment_7");
					put(grammarAccess.getXBooleanLiteralAccess().getIsTrueAssignment_1_1(), "rule__XBooleanLiteral__IsTrueAssignment_1_1");
					put(grammarAccess.getXIntLiteralAccess().getValueAssignment_1(), "rule__XIntLiteral__ValueAssignment_1");
					put(grammarAccess.getXStringLiteralAccess().getValueAssignment_1(), "rule__XStringLiteral__ValueAssignment_1");
					put(grammarAccess.getXTypeLiteralAccess().getTypeAssignment_3(), "rule__XTypeLiteral__TypeAssignment_3");
					put(grammarAccess.getXThrowExpressionAccess().getExpressionAssignment_2(), "rule__XThrowExpression__ExpressionAssignment_2");
					put(grammarAccess.getXReturnExpressionAccess().getExpressionAssignment_2(), "rule__XReturnExpression__ExpressionAssignment_2");
					put(grammarAccess.getXTryCatchFinallyExpressionAccess().getExpressionAssignment_2(), "rule__XTryCatchFinallyExpression__ExpressionAssignment_2");
					put(grammarAccess.getXTryCatchFinallyExpressionAccess().getCatchClausesAssignment_3_0_0(), "rule__XTryCatchFinallyExpression__CatchClausesAssignment_3_0_0");
					put(grammarAccess.getXTryCatchFinallyExpressionAccess().getFinallyExpressionAssignment_3_0_1_1(), "rule__XTryCatchFinallyExpression__FinallyExpressionAssignment_3_0_1_1");
					put(grammarAccess.getXTryCatchFinallyExpressionAccess().getFinallyExpressionAssignment_3_1_1(), "rule__XTryCatchFinallyExpression__FinallyExpressionAssignment_3_1_1");
					put(grammarAccess.getXCatchClauseAccess().getDeclaredParamAssignment_2(), "rule__XCatchClause__DeclaredParamAssignment_2");
					put(grammarAccess.getXCatchClauseAccess().getExpressionAssignment_4(), "rule__XCatchClause__ExpressionAssignment_4");
					put(grammarAccess.getXFunctionTypeRefAccess().getParamTypesAssignment_0_1_0(), "rule__XFunctionTypeRef__ParamTypesAssignment_0_1_0");
					put(grammarAccess.getXFunctionTypeRefAccess().getParamTypesAssignment_0_1_1_1(), "rule__XFunctionTypeRef__ParamTypesAssignment_0_1_1_1");
					put(grammarAccess.getXFunctionTypeRefAccess().getReturnTypeAssignment_2(), "rule__XFunctionTypeRef__ReturnTypeAssignment_2");
					put(grammarAccess.getJvmParameterizedTypeReferenceAccess().getTypeAssignment_0(), "rule__JvmParameterizedTypeReference__TypeAssignment_0");
					put(grammarAccess.getJvmParameterizedTypeReferenceAccess().getArgumentsAssignment_1_1(), "rule__JvmParameterizedTypeReference__ArgumentsAssignment_1_1");
					put(grammarAccess.getJvmParameterizedTypeReferenceAccess().getArgumentsAssignment_1_2_1(), "rule__JvmParameterizedTypeReference__ArgumentsAssignment_1_2_1");
					put(grammarAccess.getJvmWildcardTypeReferenceAccess().getConstraintsAssignment_2_0(), "rule__JvmWildcardTypeReference__ConstraintsAssignment_2_0");
					put(grammarAccess.getJvmWildcardTypeReferenceAccess().getConstraintsAssignment_2_1(), "rule__JvmWildcardTypeReference__ConstraintsAssignment_2_1");
					put(grammarAccess.getJvmUpperBoundAccess().getTypeReferenceAssignment_1(), "rule__JvmUpperBound__TypeReferenceAssignment_1");
					put(grammarAccess.getJvmUpperBoundAndedAccess().getTypeReferenceAssignment_1(), "rule__JvmUpperBoundAnded__TypeReferenceAssignment_1");
					put(grammarAccess.getJvmLowerBoundAccess().getTypeReferenceAssignment_1(), "rule__JvmLowerBound__TypeReferenceAssignment_1");
					put(grammarAccess.getJvmTypeParameterAccess().getNameAssignment_0(), "rule__JvmTypeParameter__NameAssignment_0");
					put(grammarAccess.getJvmTypeParameterAccess().getConstraintsAssignment_1_0_0(), "rule__JvmTypeParameter__ConstraintsAssignment_1_0_0");
					put(grammarAccess.getJvmTypeParameterAccess().getConstraintsAssignment_1_0_1(), "rule__JvmTypeParameter__ConstraintsAssignment_1_0_1");
					put(grammarAccess.getJvmTypeParameterAccess().getConstraintsAssignment_1_1(), "rule__JvmTypeParameter__ConstraintsAssignment_1_1");
					put(grammarAccess.getBookingAccess().getUnorderedGroup_1_1(), "rule__Booking__UnorderedGroup_1_1");
					put(grammarAccess.getDefinitionsAccess().getUnorderedGroup_1_2_1(), "rule__Definitions__UnorderedGroup_1_2_1");
					put(grammarAccess.getExtendAccess().getUnorderedGroup_3_1(), "rule__Extend__UnorderedGroup_3_1");
					put(grammarAccess.getJournalAttributesAccess().getUnorderedGroup_1_2_1(), "rule__JournalAttributes__UnorderedGroup_1_2_1");
					put(grammarAccess.getJournalEntryAccess().getUnorderedGroup_3_1(), "rule__JournalEntry__UnorderedGroup_3_1");
					put(grammarAccess.getResourceAttributesAccess().getUnorderedGroup_1_2_1(), "rule__ResourceAttributes__UnorderedGroup_1_2_1");
					put(grammarAccess.getScenarioAccess().getUnorderedGroup_3_1(), "rule__Scenario__UnorderedGroup_3_1");
					put(grammarAccess.getShiftAccess().getUnorderedGroup_3_1(), "rule__Shift__UnorderedGroup_3_1");
					put(grammarAccess.getTagFileAccess().getUnorderedGroup_3(), "rule__TagFile__UnorderedGroup_3");
					put(grammarAccess.getTaskAttributesAccess().getUnorderedGroup_1_2_1(), "rule__TaskAttributes__UnorderedGroup_1_2_1");
					put(grammarAccess.getTaskDependencyAccess().getUnorderedGroup_1_1(), "rule__TaskDependency__UnorderedGroup_1_1");
				}
			};
		}
		return nameMappings.get(element);
	}
	
	@Override
	protected Collection<FollowElement> getFollowElements(AbstractInternalContentAssistParser parser) {
		try {
			org.jjflyboy.tjpeditor.ui.contentassist.antlr.internal.InternalProjectParser typedParser = (org.jjflyboy.tjpeditor.ui.contentassist.antlr.internal.InternalProjectParser) parser;
			typedParser.entryRuleGlobal();
			return typedParser.getFollowElements();
		} catch(RecognitionException ex) {
			throw new RuntimeException(ex);
		}		
	}
	
	@Override
	protected String[] getInitialHiddenTokens() {
		return new String[] { "RULE_WS", "RULE_ML_COMMENT", "RULE_SL_COMMENT" };
	}
	
	public ProjectGrammarAccess getGrammarAccess() {
		return this.grammarAccess;
	}
	
	public void setGrammarAccess(ProjectGrammarAccess grammarAccess) {
		this.grammarAccess = grammarAccess;
	}
}
